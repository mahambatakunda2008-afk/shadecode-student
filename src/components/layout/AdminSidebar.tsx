...
export function AdminSidebar() {
  ...
  function navItemClass(active: boolean) {
    return cn(
      "group relative flex items-center gap-2.5 min-h-[40px] px-3 py-[11px] rounded-[9px] overflow-hidden text-[13px] leading-none transition-all duration-200 outline-none",
      active
  ...
  return (
    <aside className="flex h-full w-full flex-col overflow-hidden border-r border-[var(--card-border)] bg-[var(--surface)]">
      <div className="flex items-center px-4 pt-4 pb-4 flex-shrink-0">
        <Link href="/" aria-label="Shadecode Student home">
          <BrandLockup compact />
        </Link>
      </div>
      ...
      <div className="mx-3 mb-3 flex-shrink-0 flex items-center gap-2 rounded-2xl border border-[var(--card-border)] bg-[var(--surface-2)] px-3 py-3 shadow-sm">
        ...
        <span className="ssc-nav-label text-[11px] font-semibold text-[var(--foreground)]">Admin access</span>
      </div>
