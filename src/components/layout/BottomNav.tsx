...
const PRIMARY_ITEMS = [
  ...
  { href: "/tasks",      label: "Tasks",    icon: CheckSquare,   badge: "3",  urgent: false },
...
const MORE_ITEMS = [
  ...
  { href: "/exams",        label: "Exams",       icon: BookOpen,  badge: "2d", urgent: true },
...
export function BottomNav() {
  ...
  const anyMoreActive = MORE_ITEMS.some(({ href }) => isActive(href));

  return (
    <>
      {/* ── Bottom bar ──────────────────────────────────────────────────── */}
      <nav className="w-full bg-[#0e0e18]/95 backdrop-blur-xl
        border-t border-white/[0.07] flex items-stretch px-1">

        {PRIMARY_ITEMS.map(({ href, label, icon: Icon, badge, urgent }) => {
      ...
      {open && (
        <div
          className="fixed bottom-0 left-0 right-0 z-[9999]
            bg-[#0a0a10]/98 backdrop-blur-2xl
            border-t border-white/[0.08]
            rounded-t-[24px] px-5 pt-5 pb-8"
        ...
        >
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <button
              onClick={() => setOpen(false)}
              className="w-8 h-8 rounded-full bg-white/[0.06]
          ...
          <div className="grid grid-cols-3 gap-3">
            {MORE_ITEMS.map(({ href, label, icon: Icon, badge, urgent }) => {
              const active = isActive(href);
              return (
                <Link
                  key={href}
