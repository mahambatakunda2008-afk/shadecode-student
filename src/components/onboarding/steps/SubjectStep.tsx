...
export function SubjectsStep({ data, onUpdate, onNext, onBack }: StepProps) {
  ...
  const group = level === 'primary' ? SUBJECTS.primary : level === 'university' || level === 'tvet' || level === 'professional' ? SUBJECTS.tertiary : SUBJECTS.secondary;
  ...
  return (
    <div style={{ opacity: in_ ? 1 : 0, transform: in_ ? 'translateY(0)' : 'translateY(10px)', transition: 'opacity .3s ease, transform .3s ease' }}>
      <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--primary)', marginBottom: 5 }}>Your subjects</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7, maxHeight: 300, overflowY: 'auto', paddingRight: 3 }}>
        {group.map(([id, label, emoji]) => {
          ...
          return <button key={id} type="button" onClick={() => toggle(id)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px', borderRadius: 10, textAlign: 'left', cursor: 'pointer', background: active ? 'color-mix(in srgb, var(--primary) 12%, var(--card))' : 'var(--muted)', border: active ? '1px solid var(--primary)' : '1px solid var(--card-border)' }}><span>{emoji}</span><span style={{ fontSize: 12, fontWeight: 600, color: active ? 'var(--primary)' : 'var(--foreground)' }}>{label}</span>{active && <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--primary)' }}>✓</span>}</button>;
        })}
      </div>
      ...
      {error && <p role="alert" style={{ fontSize: 12, color: '#ef4444', marginTop: 10 }}>{error}</p>}
