...
export function SubjectsStep({ data, onUpdate, onNext, onBack }: StepProps) {
  ...
  const handleNext = () => { if (!level) return setError('Choose your path first.'); if (!selected.length) return setError('Choose at least one subject.'); onUpdate({ subjects: selected }); onNext(); };
  return <div style={{ opacity: in_ ? 1 : 0, transform: in_ ? 'translateY(0)' : 'translateY(8px)', transition: 'opacity .25s ease, transform .25s ease' }}>
    <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.11em', textTransform: 'uppercase', color: '#22D3EE', marginBottom: 6 }}>03 · Subjects</p>
    <h2 style={{ fontSize: 23, lineHeight: 1.15, fontWeight: 720, color: '#F5F7FA', marginBottom: 6 }}>{level === 'primary' ? 'What do you want to explore?' : 'What are you studying?'}</h2>
    <p style={{ fontSize: 13, lineHeight: 1.5, color: '#AAB2BF', marginBottom: 18 }}>Pick the subjects you want Shadecode to focus on first. You can add more later.</p>
