...
export function AcademicContextStep({ data, onUpdate, onNext, onBack }: StepProps) {
  ...
  const title = primary ? 'Set your learning level' : secondary ? 'Set your school context' : 'Set your study context';
  const subtitle = primary ? 'This keeps activities at the right level and language.' : secondary ? 'This helps us use the right subjects, syllabus and difficulty.' : 'This helps us organize learning around your actual programme.';
  const handleNext = () => {
    if (!level) return setError('Go back and choose your path first.');
    if (!data.yearLevel?.trim()) return setError(primary ? 'Choose your grade.' : tertiary ? 'Enter your year or level.' : 'Enter your school level or year.');
  ...
  return (
    <div>
      <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.11em', textTransform: 'uppercase', color: '#22D3EE', marginBottom: 6 }}>Your setup</p>
      ...
      {secondary && <div style={{ display: 'grid', gap: 13 }}>
        <label style={labelStyle}>{level === 'a-level' ? 'Level' : 'School level / year'}<input value={data.yearLevel ?? ''} onChange={e => { onUpdate({ yearLevel: e.target.value }); setError(''); }} placeholder={level === 'a-level' ? 'e.g. AS Level' : 'e.g. Form 3'} style={{ ...inputStyle, marginTop: 7 }} /></label>
