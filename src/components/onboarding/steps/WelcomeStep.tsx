...
export function WelcomeStep({ data, onUpdate, onNext }: StepProps) {
  ...
  const openDemo = () => {
    setDemoOpen(true);
    void trackEvent('onboarding_demo_opened', { demo: 'instant_math' });
  };
  ...
  return (
    <div style={{ opacity: in_ ? 1 : 0, transform: in_ ? 'translateY(0)' : 'translateY(10px)', transition: 'opacity 0.3s ease, transform 0.3s ease' }}>
      ...
      <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.38)', marginBottom: 18 }}>Two quick details, then Shadecode can personalize your first session.</p>
      ...
      {demoOpen && (
        <div style={{ marginBottom: 20, padding: 14, borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input value={demoAnswer} onChange={(e) => setDemoAnswer(e.target.value)} inputMode="numeric" placeholder="Answer" style={{ flex: 1, minWidth: 0, padding: '9px 11px', borderRadius: 9, background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.1)', color: 'white' }} />
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <input type="text" value={name} autoFocus maxLength={40} placeholder="Your name" onChange={e => { setName(e.target.value); setError(''); }} onKeyDown={e => e.key === 'Enter' && handleNext()} style={{ width: '100%', boxSizing: 'border-box', padding: '10px 14px', borderRadius: 10, fontSize: 13, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.9)', outline: 'none' }} />
      </div>
