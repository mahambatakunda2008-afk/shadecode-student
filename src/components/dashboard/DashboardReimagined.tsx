    ...
    <main className="min-h-full bg-background">
      <div className="mx-auto w-full max-w-[1440px] px-4 pb-12 pt-6 sm:px-6 lg:px-8">
        <header className="mb-7 flex flex-col gap-1 sm:mb-8">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Academic command center</p>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(300px,0.55fr)]">
          <div className="space-y-6">
            <section aria-labelledby="today-heading"><SectionTitle kicker="Keep moving" title="Today’s plan" />
              <Card><CardContent className="p-2 sm:p-3">{intel?.recommendations?.slice(0, 4).length ? intel.recommendations.slice(0, 4).map((item: any, i: number) => <button key={item.id || i} type="button" onClick={() => router.push("/learn")} className="flex w-full items-center gap-3 rounded-xl p-3 text-left transition-colors hover:bg-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-muted"><Target className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block text-sm font-medium">{item.title}</span><span className="mt-0.5 block line-clamp-1 text-xs text-muted-foreground">{item.description}</span></span><ArrowRight className="h-4 w-4 shrink-0 text-muted-foreground" /></button>) : <p className="p-5 text-sm text-muted-foreground">Your plan will appear as Shadecode gathers more learning activity.</p>}</CardContent></Card>
            </section>

            <section aria-labelledby="upcoming-heading"><SectionTitle icon={<CalendarClock />} title="Next up" /><Card><CardContent className="divide-y p-0">{exams.length ? exams.map(exam => <button key={exam.id} type="button" onClick={() => router.push("/tasks")} className="flex w-full items-center justify-between gap-4 p-4 text-left hover:bg-muted/50"><span><span className="block text-sm font-medium">{exam.subject}</span><span className="text-xs text-muted-foreground">{new Date(exam.exam_date).toLocaleDateString()}</span></span><ArrowRight className="h-4 w-4 text-muted-foreground" /></button>) : <p className="p-5 text-sm text-muted-foreground">No upcoming assessments to show.</p>}</CardContent></Card></section>
          </div>
