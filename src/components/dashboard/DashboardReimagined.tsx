...
/**
 * Dashboard data loading is split into two independent boundaries, per
 * docs/DASHBOARD_REDESIGN_SPEC.md section 7 / DASHBOARD_COMPONENT_CONTRACT.md's
 * async contract ("a failed secondary section must not blank the entire
 * dashboard"). Previously the whole page went through one
 * getStudentIntelligence() call that Promise.all'd all four underlying
 * services together and returned null if ANY of them failed -- so a slow
 * or failing AI-touching intelligence call could blank the entire page,
 * including momentum/subjects/exams, which are just fast DB reads that
 * had already succeeded. Verified by reading student-intelligence/index.ts
 * directly on 2026-08-15, not assumed.
 *
 * "core" = progress + performance + activity: fast, reliable, DB-backed,
 * fetched together since their success/failure is strongly correlated
 * (same data source, same low failure risk).
 * "intel" = the intelligence engine: the one call that can touch an AI
 * provider and is meaningfully slower/more failure-prone, so it gets its
 * own independent loading/error/retry boundary.
 */
export default function DashboardReimagined() {
  ...
  const [userName, setUserName] = useState("Student"); const [userId, setUserId] = useState<string | null>(null);

  const [core, setCore] = useState<{ progress: StudentProgress; performance: StudentPerformance; activity: StudentActivity } | null>(null);
  const [coreLoading, setCoreLoading] = useState(true);
  const [coreError, setCoreError] = useState<string | null>(null);

  const [intel, setIntel] = useState<StudentIntelligenceData | null>(null);
  ...
  const loadCore = async (id: string) => {
    ...
    try {
      const [progress, performance, activity] = await withTimeout(
        Promise.all([
      ...
      setCore({ progress: progress.data, performance: performance.data, activity: activity.data });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setUserId(auth.user.id); setUserName(auth.user.user_metadata?.full_name?.split(" ")[0] || auth.user.email?.split("@")[0] || "Student");
      void loadCore(auth.user.id);
      void loadIntel(auth.user.id);
  ...
  const next = intel?.recommendations?.[0]; const weak = intel?.weakAreas?.[0]; const insight = intel?.insights?.[0];
  const progressPct = core?.progress?.overallCompletion; const score = core?.performance?.trends?.averageScore;
  const subjects = (core?.progress?.subjects || []).slice(0, 6);
  const recentLessons = (core?.progress?.lessons || []).filter((lesson: any) => lesson.lastAttempted).sort((a: any, b: any) => new Date(b.lastAttempted).getTime() - new Date(a.lastAttempted).getTime()).slice(0, 3);
  const activeMinutes = core?.activity?.patterns?.averageDailyStudyTime; const recommendationCount = intel?.recommendations?.length || 0;
  // Recommendation has no `subject` field (verified against intelligenceEngine's
  // source -- it never sets one), so the accent can only come from `weak`. Was
  // silently `next?.subject` (always undefined) before this file had real types.
  const nextAccent = accentFor(weak?.subject);

  const greeting = getWelcomeContext({ core, coreLoading, exams });
    ...
    <main className="dashboard-command-center min-h-full">
      ...
      <motion.div className="dashboard-shell mx-auto w-full max-w-[1480px] px-4 pb-14 pt-5 sm:px-6 lg:px-8" variants={containerVariants} initial="hidden" animate="show">
        <motion.header className="dashboard-header" variants={itemVariants}>
          <div><p className="dashboard-kicker"><Sparkles className="h-3.5 w-3.5" /> Your learning cockpit</p><h1>{greeting}</h1><p>One place to see what needs your attention, what is improving, and what to do next.</p></div>
        ...
        {coreError ? (
          <motion.div variants={itemVariants} className="dashboard-core-error" role="alert">
            <p>{coreError}</p>
            <button type="button" className="dashboard-outline-action" disabled={!userId || coreLoading} onClick={() => userId && loadCore(userId)}>Retry <RotateCcw className="h-4 w-4" /></button>
          </motion.div>
        ) : (
          <>
            ...
            <motion.aside className="dashboard-side-column" variants={itemVariants}><section aria-labelledby="focus-heading"><SectionTitle kicker="High impact" title="Focus area" /><Card className="dashboard-focus" style={{ ["--focus-accent" as string]: accentFor(weak?.subject) }}><CardContent className="p-5">{intelError ? <p className="text-sm leading-6 text-muted-foreground">Cortex is unavailable right now -- see the Next Best Move card above to retry.</p> : weak ? <><div className="dashboard-focus-top"><span className="dashboard-focus-chip">{weak.subject}</span><span className={`dashboard-severity severity-${weak.severity}`}>{weak.severity}</span></div><h3 id="focus-heading">{weak.topic}</h3><div className="dashboard-score-line"><span>Current signal</span><strong>{Math.round(weak.score)}%</strong></div><div className="dashboard-score-track"><span style={{ width: `${Math.max(0, Math.min(100, weak.score))}%` }} /></div><p>Focused practice here is likely to have the biggest near-term payoff.</p><button type="button" className="dashboard-outline-action" onClick={() => router.push(`/learn?subject=${encodeURIComponent(weak.subject)}&topic=${encodeURIComponent(weak.topic)}`)}>Work on {weak.topic} <ArrowRight className="h-4 w-4" /></button></> : <p id="focus-heading" className="text-sm leading-6 text-muted-foreground">Complete more practice and Cortex will surface the area where your effort can have the most impact.</p>}</CardContent></Card></section>
              ...
              <section aria-labelledby="cortex-heading"><SectionTitle icon={<BrainCircuit className="text-primary" />} title="Cortex signal" /><Card className="dashboard-cortex"><CardContent className="p-5">{intelError ? <><h3 id="cortex-heading">Cortex needs a moment</h3><p>{intelError}</p><button type="button" className="dashboard-outline-action" disabled={!userId || intelLoading} onClick={() => userId && loadIntel(userId)}>Retry <RotateCcw className="h-4 w-4" /></button></> : insight ? <><span className="dashboard-signal-label">{insight.type}</span><h3 id="cortex-heading">{insight.title || "A useful signal"}</h3><p>{insight.content}</p></> : <><h3 id="cortex-heading">Still learning your pattern</h3><p>Keep learning. Cortex becomes more useful as your activity and performance history grow.</p></>}</CardContent></Card></section>
            </motion.aside>
            </div>
          </>
        )}
...
/**
 * Grounded entirely in real, already-fetched data -- never fabricates
 * personalization (spec section 4.A / 11). Falls back to a neutral
 * time-of-day greeting when core data hasn't loaded yet or no stronger
 * signal exists.
 */
function getWelcomeContext({
  ...
  core: { progress: StudentProgress; activity: StudentActivity } | null;
  coreLoading: boolean;
  exams: any[];
}): string {
  ...
  if (coreLoading || !core) return `${timeOfDayGreeting}.`;

  // Brand new user: no subject progress recorded at all yet.
  if (!core.progress.subjects || core.progress.subjects.length === 0) {
    return "Welcome to Shadecode Student.";
  }

  // An exam within the next 7 days is the most actionable, urgent context.
  const soonExam = exams
    .map((e) => ({ ...e, daysUntil: Math.ceil((new Date(e.exam_date).getTime() - Date.now()) / 86_400_000) }))
    .filter((e) => e.daysUntil >= 0 && e.daysUntil <= 7)
    .sort((a, b) => a.daysUntil - b.daysUntil)[0];
  if (soonExam) {
    const when = soonExam.daysUntil === 0 ? "today" : soonExam.daysUntil === 1 ? "tomorrow" : `in ${soonExam.daysUntil} days`;
    return `Your ${soonExam.subject} exam is ${when}.`;
  }

  const lastStudyDate = core.activity?.streak?.lastStudyDate;
  const daysSinceStudy = lastStudyDate ? Math.floor((Date.now() - new Date(lastStudyDate).getTime()) / 86_400_000) : null;
  const streak = core.activity?.streak?.currentStreak ?? 0;

  if (streak > 0 && daysSinceStudy !== null && daysSinceStudy <= 1) {
    return `${timeOfDayGreeting} -- you're on a ${streak}-day streak.`;
  }

  if (daysSinceStudy !== null && daysSinceStudy >= 2) {
    return "Welcome back -- let's pick up where you left off.";
  }
