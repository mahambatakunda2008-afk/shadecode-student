...
export default function DashboardReimagined() {
  ...
  const progress = data?.progress?.overallCompletion; const score = data?.performance?.trends?.averageScore; const streak = data?.activity?.streak?.currentStreak;
    ...
    <main className="dashboard-command-center min-h-full">
      ...
      <div className="dashboard-shell mx-auto w-full max-w-[1480px] px-4 pb-14 pt-5 sm:px-6 lg:px-8">
        <header className="dashboard-header">
          ...
          <div className="dashboard-header-stat"><span>Study rhythm</span><strong>{activeMinutes == null ? "Building" : `${Math.round(activeMinutes)}m`}</strong><small>average daily</small></div>
        </header>
...
function Metric({ icon, label, value, detail }: { icon: React.ReactNode; label: string; value: string; detail: string }) { return <div className="dashboard-metric"><span className="dashboard-metric-icon">{icon}</span><span><small>{label}</small><strong>{value}</strong><em>{detail}</em></span></div>; }
function SubjectTile({ subject, onClick }: { subject: any; onClick: () => void }) { const accent = accentFor(subject.subject); const percent = Math.max(0, Math.min(100, Number(subject.completionPercentage) || 0)); return <button type="button" className="dashboard-subject-tile" onClick={onClick} style={{ ["--subject-accent" as string]: accent }}><span className="dashboard-subject-orb" /><span className="dashboard-subject-copy"><strong>{subject.subject}</strong><small>{subject.completedLessons || 0} of {subject.totalLessons || 0} lessons</small></span><span className="dashboard-progress-ring" style={{ ["--progress" as string]: `${percent * 3.6}deg` }}><b>{Math.round(percent)}%</b></span></button>; }
