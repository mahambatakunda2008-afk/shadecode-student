...
export default function DiscoveryActivity({ activityId }: { activityId: string }) {
  ...
  async function next() {
    ...
    if (nextIndex < questions.length) { setCorrect(nextCorrect); setIndex(nextIndex); setSelected(null); save(nextIndex, nextCorrect, false); return; }
    const attemptCount = (progress?.attempt_count ?? 0) + 1;
    const score = Math.round((nextCorrect / questions.length) * 100);
    const finishedAt = new Date().toISOString();
    setCorrect(nextCorrect); setFinished(true); setIndex(questions.length); save(questions.length, nextCorrect, true);
    primaryActivityCompletedEvent(activity.id, activity.subject, activity.topic, activity.skill, { evidenceScore: score, percentage: score, offline });
