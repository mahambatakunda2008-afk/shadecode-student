...
export default function ExamShareCard({
  ...
  const exportPayload = {
    ...
    timeSeconds: result.time_taken,
    xpEarned: result.xp_earned,
    completedAt: result.created_at,
  }

  const handleExport = () => {
    downloadExport(`shadecode-${result.subject.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-result`, exportPayload, 'json')
  ...
  const handleCopyResult = async () => {
    ...
    void trackEvent('result_shared', { channel: 'copy', resultId: result.id })
    setTimeout(() => setCopiedResult(false), 2000)
  }

  const handleWhatsAppResult = () => {
    void trackEvent('result_shared', { channel: 'whatsapp', resultId: result.id })
  ...
  const handleCopyChallenge = async () => {
    ...
    void trackEvent('challenge_shared', { channel: 'copy', resultId: result.id })
    setTimeout(() => setCopiedChallenge(false), 2000)
  }

  const handleWhatsAppChallenge = () => {
    if (!challengeShareText) return
    void trackEvent('challenge_shared', { channel: 'whatsapp', resultId: result.id })
      ...
      <div className="relative rounded-2xl border border-[var(--card-border)] bg-[var(--card)] overflow-hidden shadow-2xl">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-[var(--card-border)]">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold tracking-widest text-indigo-400 uppercase">◈ Shadecode Student</span>
        ...
        <div className="px-6 py-6 space-y-6">
          <div>
            ...
            <p className="text-xl font-bold text-[var(--foreground)]">{result.subject}{result.topic && <span className="text-[var(--muted-foreground)] font-normal text-base ml-2">· {result.topic}</span>}</p>
          </div>

          <div className="flex items-end gap-6">
            <div>
              <p className="text-xs text-[var(--muted-foreground)] uppercase tracking-wider mb-1">Score</p>
              <p className="text-5xl font-black text-[var(--foreground)]">{result.score}<span className="text-2xl text-[var(--muted-foreground)] font-semibold">/{result.total}</span></p>
              <p className="text-sm text-[var(--muted-foreground)] mt-1">{percentage}%</p>
            </div>
            <div className="ml-auto text-right"><p className="text-xs text-[var(--muted-foreground)] uppercase tracking-wider mb-1">Grade</p><p className={cn('text-6xl font-black', gradeColor)}>{result.grade}</p></div>
          </div>

          <div className="grid grid-cols-3 gap-3">
            {[{ label: 'Time', value: formatTime(result.time_taken) }, { label: 'XP Earned', value: `+${result.xp_earned}` }, { label: 'Questions', value: `${result.question_count}Q` }].map(({ label, value }) => (
              <div key={label} className="rounded-xl bg-[var(--surface-2)] border border-[var(--card-border)] px-3 py-3 text-center"><p className="text-xs text-[var(--muted-foreground)] mb-1">{label}</p><p className="text-sm font-semibold text-[var(--foreground)]">{value}</p></div>
            ))}
          </div>
        ...
        <div className="px-6 pb-6 space-y-3">
          <div className="grid grid-cols-3 gap-2">
            <button onClick={handleCopyResult} className={cn('flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-xs font-medium border', copiedResult ? 'bg-emerald-500/15 border-emerald-500/30 text-emerald-400' : 'bg-[var(--surface-2)] border-[var(--card-border)] text-[var(--muted-foreground)] hover:bg-[var(--muted)] hover:text-[var(--foreground)]')}>
              {copiedResult ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}{copiedResult ? 'Copied!' : 'Copy Link'}
            </button>
          ...
          {!challengeUrl ? (
            <button onClick={handleCreateChallenge} disabled={creatingChallenge} className="w-full flex items-center justify-center gap-2 rounded-xl bg-orange-500/10 border border-orange-500/25 text-orange-400 hover:bg-orange-500/18 font-semibold py-3 px-4 text-sm disabled:opacity-50">
              {creatingChallenge ? <><Loader2 className="w-4 h-4 animate-spin" /> Creating challenge…</> : <><Swords className="w-4 h-4" /> Challenge a Friend 🔥</>}
            </button>
          ) : (
            <div className="rounded-xl bg-orange-500/8 border border-orange-500/20 p-4 space-y-3">
              <div className="flex items-center gap-2 mb-1"><Swords className="w-3.5 h-3.5 text-orange-400" /><p className="text-xs font-bold text-orange-400 uppercase tracking-wider">Challenge Link Ready</p></div>
              <p className="text-xs text-[var(--muted-foreground)] truncate font-mono bg-[var(--surface-2)] rounded-lg px-3 py-2">{fullChallengeUrl}</p>
              <div className="flex gap-2">
                <button onClick={handleCopyChallenge} className={cn('flex-1 flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-xs font-medium border', copiedChallenge ? 'bg-emerald-500/15 border-emerald-500/30 text-emerald-400' : 'bg-[var(--surface-2)] border-[var(--card-border)] text-[var(--muted-foreground)] hover:bg-[var(--muted)]')}>{copiedChallenge ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}{copiedChallenge ? 'Copied!' : 'Copy'}</button>
                <button onClick={handleWhatsAppChallenge} className="flex-1 flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-xs font-medium bg-[#25D366]/15 border border-[#25D366]/30 text-[#25D366] hover:bg-[#25D366]/25"><Share2 className="w-3.5 h-3.5" />WhatsApp</button>
              </div>
            </div>
          )}

          {showSignupCta && <button onClick={handleChallenge} className="w-full rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold py-3 px-4 text-sm">Try Shadecode Student Free →</button>}
        </div>
      </div>
    </div>
  )
