...
export default function ShareCard({ title, text, shareType, className = "" }: ShareCardProps) {
  ...
  const getUrl = () => typeof window === "undefined" ? "https://shadecodestudent.vercel.app" : window.location.href;
  ...
  const copy = async () => {
    try {
      ...
      setCopied(true);
      void trackEvent("share_completed", { shareType, method: "copy" });
  ...
  const nativeShare = async () => {
    ...
    try { await navigator.share({ title, text: shareText, url: getUrl() }); void trackEvent("share_completed", { shareType, method: "native" }); }
    catch (error) { if (error instanceof DOMException && error.name === "AbortError") return; }
  };

  const whatsapp = () => { window.open(`https://wa.me/?text=${encodeURIComponent(`${shareText}\n${getUrl()}`)}`, "_blank", "noopener,noreferrer"); void trackEvent("share_started", { shareType, method: "whatsapp" }); };

  return <div className={`rounded-2xl border border-[var(--card-border)] bg-[var(--surface-2)] p-4 ${className}`}>
    <div className="mb-3 flex items-center gap-2"><Share2 size={17} className="text-[var(--primary)]" /><strong>Share your progress</strong></div>
    <p className="mb-4 text-sm leading-6 text-[var(--muted-foreground)]">{text}</p>
    <div className="flex flex-wrap gap-2"><button type="button" onClick={nativeShare} className="inline-flex items-center gap-2 rounded-xl bg-[var(--primary)] px-3 py-2 text-sm font-semibold text-white"><Share2 size={15} /> Share</button><button type="button" onClick={whatsapp} className="inline-flex items-center gap-2 rounded-xl border border-[var(--card-border)] px-3 py-2 text-sm font-semibold"><MessageCircle size={15} /> WhatsApp</button><button type="button" onClick={copy} className="inline-flex items-center gap-2 rounded-xl border border-[var(--card-border)] px-3 py-2 text-sm font-semibold">{copied ? <Check size={15} /> : <Copy size={15} />} {copied ? "Copied" : "Copy"}</button></div>
  </div>;
}
