...
export default function CortexGenerationIndicator() {
  ...
  const detail = primary.status === "queued"
    ...
    : primary.status === "warming"
      ...
      : primary.engine === "cloud"
        ? "Using cloud enhancement"
        : "Running on this device";
  const EngineIcon = primary.engine === "cloud" ? Cloud : Cpu;
      ...
      <div className="flex items-center gap-3 rounded-2xl border border-[var(--card-border)] bg-[var(--card)] px-3.5 py-3 shadow-lg backdrop-blur">
        ...
        <span className="min-w-0">
          ...
          <span className="mt-0.5 flex items-center gap-1.5 text-xs text-[var(--muted-foreground)]">
            {primary.status !== "queued" && primary.status !== "warming" && <EngineIcon className="h-3.5 w-3.5" aria-hidden="true" />}
            {detail}
          </span>
        </span>
