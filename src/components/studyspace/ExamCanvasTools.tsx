...
export default function ExamCanvasTools({ storageKey, onChange, onRecognized }: Props) {
  ...
  const resize = () => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const oldWidth = canvas.clientWidth || 1;
    const oldHeight = canvas.clientHeight || 1;
    const dpr = Math.min(window.devicePixelRatio || 1, 2);
    const newWidth = canvas.clientWidth || oldWidth;
    const newHeight = canvas.clientHeight || oldHeight;
    const sx = newWidth / oldWidth;
    const sy = newHeight / oldHeight;
    if (sx !== 1 || sy !== 1) strokesRef.current = strokesRef.current.map((s) => ({ x: s.x.map((v) => v * sx), y: s.y.map((v) => v * sy) }));
    canvas.width = Math.max(1, Math.round(newWidth * dpr));
    canvas.height = Math.max(1, Math.round(newHeight * dpr));
    redraw();
  };
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    try {
      const saved = localStorage.getItem(`${storageKey}:strokes`);
  ...
  const down = (event: PointerEvent<HTMLCanvasElement>) => {
    ...
    const p = point(event);
    lastRef.current = p;
    currentRef.current = { x: [p.x], y: [p.y] };
  };
  const move = (event: PointerEvent<HTMLCanvasElement>) => {
    if (!drawingRef.current || !lastRef.current || !currentRef.current) return;
    const p = point(event);
    currentRef.current.x.push(p.x); currentRef.current.y.push(p.y); lastRef.current = p;
    ...
    ctx.strokeStyle = tool === "eraser" ? "#ffffff" : tool === "highlighter" ? "#f5d90a" : "#111827";
    ctx.globalAlpha = tool === "highlighter" ? 0.32 : 1;
    ctx.globalCompositeOperation = tool === "eraser" ? "destination-out" : "source-over";
    ctx.lineWidth = tool === "eraser" ? 20 : tool === "highlighter" ? 12 : 2.4;
    ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.stroke(); ctx.globalAlpha = 1; ctx.globalCompositeOperation = "source-over";
  };
  const up = () => {
    ...
    if (stroke.x.length > 1 && strokesRef.current.length < MAX_STROKES) strokesRef.current.push(stroke);
    lastRef.current = null; setInk(strokesRef.current.length > 0); persist();
  };
  ...
  const tidy = () => {
    ...
    historyRef.current.push(clone(strokesRef.current));
    const yValues = strokesRef.current.map((s) => s.y.reduce((a, b) => a + b, 0) / s.y.length);
    const sorted = [...yValues].sort((a, b) => a - b);
    const tolerance = 12;
    const bands: number[] = [];
    for (const y of sorted) { const last = bands.at(-1); if (last === undefined || Math.abs(y - last) > tolerance) bands.push(y); }
    strokesRef.current = strokesRef.current.map((s) => {
      const avg = s.y.reduce((a, b) => a + b, 0) / s.y.length;
      const nearest = bands.reduce((best, band) => Math.abs(band - avg) < Math.abs(best - avg) ? band : best, bands[0] ?? avg);
      const delta = nearest - avg;
      return { x: [...s.x], y: s.y.map((y) => y + delta) };
    });
    redraw(); persist(); setMessage("Working aligned");
  };
  const recognize = async () => {
    if (!strokesRef.current.length) return;
    setRecognizing(true); setMessage("");
    try {
      ...
      onRecognized?.({ text: result.text ?? "", latex: result.latex ?? "", confidence: result.confidence ?? null });
      setMessage(result.text || result.latex ? "Recognized" : "No handwriting detected");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Recognition unavailable"); }
