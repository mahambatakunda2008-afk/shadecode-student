...
export default function ExamCanvasTools({ storageKey, onChange, onRecognized }: Props) {
  ...
  const clone = (value: Stroke[]) => value.map((stroke) => ({ x: [...stroke.x], y: [...stroke.y] }));
  const snapshot = () => canvasRef.current?.toDataURL("image/png") ?? "";
  const persist = () => { const data = snapshot(); try { localStorage.setItem(storageKey, data); localStorage.setItem(`${storageKey}:strokes`, JSON.stringify(strokesRef.current)); } catch {} onChange?.(data); };
  const redraw = () => {
    ...
    for (const stroke of strokesRef.current) { if (stroke.x.length < 2) continue; ctx.beginPath(); ctx.moveTo(stroke.x[0], stroke.y[0]); for (let i = 1; i < stroke.x.length; i += 1) ctx.lineTo(stroke.x[i], stroke.y[i]); ctx.strokeStyle = "#111827"; ctx.globalAlpha = 1; ctx.globalCompositeOperation = "source-over"; ctx.lineWidth = 2.4; ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.stroke(); }
  };
  const resize = () => {
    const canvas = canvasRef.current; if (!canvas) return;
  ...
  const move = (event: PointerEvent<HTMLCanvasElement>) => {
    ...
    ctx.beginPath(); ctx.moveTo(currentRef.current.x.at(-2) ?? p.x, currentRef.current.y.at(-2) ?? p.y); ctx.lineTo(p.x, p.y); ctx.strokeStyle = tool === "eraser" ? "#ffffff" : tool === "highlighter" ? "#f5d90a" : "#111827"; ctx.globalAlpha = tool === "highlighter" ? 0.32 : 1; ctx.globalCompositeOperation = tool === "eraser" ? "destination-out" : "source-over"; ctx.lineWidth = tool === "eraser" ? 20 : tool === "highlighter" ? 12 : 2.4; ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.stroke(); ctx.globalAlpha = 1; ctx.globalCompositeOperation = "source-over";
  };
  ...
  const tidy = () => {
    ...
    strokesRef.current = strokesRef.current.map((s) => { const avg = s.y.reduce((a, b) => a + b, 0) / s.y.length; const nearest = bands.reduce((best, band) => Math.abs(band - avg) < Math.abs(best - avg) ? band : best, bands[0] ?? avg); const delta = nearest - avg; return { x: [...s.x], y: s.y.map((y) => y + delta) }; }); redraw(); persist(); setMessage("Working aligned");
  };
  const recognize = async () => {
    if (!strokesRef.current.length) return; setRecognizing(true); setMessage("");
    try { const response = await fetch("/api/studyspace/handwriting", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ strokes: { strokes: { x: strokesRef.current.map((s) => s.x), y: strokesRef.current.map((s) => s.y) } } }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error ?? "Recognition failed"); onRecognized?.({ text: result.text ?? "", latex: result.latex ?? "", confidence: result.confidence ?? null }); setMessage(result.text || result.latex ? "Recognized" : "No handwriting detected"); } catch (error) { setMessage(error instanceof Error ? error.message : "Recognition unavailable"); } finally { setRecognizing(false); }
  };
