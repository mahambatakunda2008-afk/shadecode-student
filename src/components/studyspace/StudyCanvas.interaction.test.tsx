...
describe("StudyCanvas interaction model", () => {
  it("creates a stable object bounds from a stroke", () => {
    expect(boundsForPoints([10, 40, 25], [20, 60, 35])).toMatchObject({ x: 10, y: 20, width: 30, height: 40 });
  });
  it("translates selected bounds without changing size", () => {
    expect(translateBounds({ x: 10, y: 20, width: 30, height: 40, rotation: 0 }, 15, -5)).toMatchObject({ x: 25, y: 15, width: 30, height: 40 });
  });
  it("resizes selected bounds", () => {
    expect(resizeBounds({ x: 10, y: 20, width: 30, height: 40, rotation: 0 }, 50, 60)).toMatchObject({ x: 10, y: 20, width: 50, height: 60 });
  });
