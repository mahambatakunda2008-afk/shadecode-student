...
export default function AcademicExperienceHero() {
  ...
  return (
    <section aria-labelledby="academic-experience-title" data-study-level={experience.stage} className="mx-4 mt-4 mb-5 overflow-hidden rounded-[22px] border border-[var(--card-border)] bg-[var(--surface)] shadow-sm md:mx-6">
      <div className="relative p-5 md:p-6">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <div className="mb-2 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.12em] text-[var(--primary)]"><Sparkles className="h-3.5 w-3.5" />{experience.label} experience</div>
          ...
          <div className="relative flex flex-wrap gap-2">
            <Link href={primaryHref} className="inline-flex items-center gap-2 rounded-xl bg-[var(--primary)] px-4 py-2.5 text-[12px] font-semibold text-white transition-transform hover:-translate-y-0.5">
              {experience.stage === "university" ? <BriefcaseBusiness className="h-4 w-4" /> : experience.stage === "early-childhood" ? <Sparkles className="h-4 w-4" /> : <BookOpen className="h-4 w-4" />}
