...
export default function CompLabWorkspace() {
  ...
  useEffect(() => {
    setLauncherId(requestedEnvironment);
        ...
        <section className="overflow-hidden rounded-3xl border border-[var(--card-border)] bg-[var(--surface)] shadow-sm">
          <header className="border-b border-[var(--card-border)] p-5 sm:p-8">
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-widest text-[var(--primary)]">Shadecode Student</p><h1 className="text-3xl font-bold text-[var(--foreground)]">Comp Lab</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--muted-foreground)]">Choose the computing environment you actually want to work in. Once opened, Comp Lab becomes that environment instead of making you stare at the whole toolbox.</p></div>
            </div>
          </header>
          <div className="p-4 sm:p-6 lg:p-8">
            <div className="mb-5 flex items-end justify-between gap-3"><div><h2 className="text-sm font-semibold text-[var(--foreground)]">Environments</h2><p className="mt-1 text-xs text-[var(--muted-foreground)]">Execution availability is technology-based, not an exam-board restriction.</p></div><span className="rounded-full border border-[var(--card-border)] px-2.5 py-1 text-[10px] text-[var(--muted-foreground)]">{COMP_LAB_ENVIRONMENTS.length} available targets</span></div>
  ...
  return (
    <main className="flex min-h-full w-full flex-col bg-[var(--background)]">
      ...
      <section className="flex-1 p-2 sm:p-5 lg:p-6">
        {selectedIsCodeWorkspace ? <><ResizableCompLabWorkspace /><CompLabIntelligencePanel /></> : selectedIsWebWorkspace ? <WebCompLabWorkspace /> : selectedIsAlgorithmWorkspace ? <AlgorithmStudioPlus /> : <div className="mx-auto max-w-4xl rounded-3xl border border-[var(--card-border)] bg-[var(--surface)] p-6 sm:p-10"><div className="flex items-start gap-4"><Code2 className="mt-0.5 h-6 w-6 shrink-0 text-[var(--primary)]" /><div><h2 className="text-lg font-semibold text-[var(--foreground)]">{selected.label} is ready for its native workflow</h2><p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">Comp Lab will use the correct runtime or artifact workflow here rather than pretending a browser runtime is equivalent. The capability layer records what is available locally and what needs a native or remote execution surface.</p><p className="mt-4 text-xs text-[var(--muted-foreground)]">Languages: {selected.languages.length ? selected.languages.join(", ") : "workspace artifact"}</p></div></div></div>}
      </section>
      <footer className="border-t border-[var(--card-border)] bg-[var(--surface)] px-4 py-3 text-xs text-[var(--muted-foreground)]"><Code2 className="mr-2 inline h-4 w-4" />Comp Lab is the student-facing computing environment. The future physical Shadecode Lab remains a separate facility concept.</footer>
    </main>
  );
}
