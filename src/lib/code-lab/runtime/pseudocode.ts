...
function valueOf(raw: string, state: ExecState): Value {
  ...
  if (text in state.vars) return state.vars[text];
  return text;
}

function numeric(v: Value) {
...
function evalExpr(raw: string, state: ExecState): Value {
  let expr = raw.trim();
  expr = expr.replace(/<>/g, "!=").replace(/\bAND\b/gi, "&&").replace(/\bOR\b/gi, "||").replace(/\bNOT\b/gi, "!").replace(/\bMOD\b/gi, "%").replace(/\bDIV\b/gi, "/");
...
function matchingEnd(lines: string[], start: number, open: RegExp, close: RegExp) {
  let depth = 0;
  for (let i = start; i < lines.length; i += 1) {
    const line = clean(lines[i]).toUpperCase();
...
function renderTrace(trace: TraceRow[]) {
  ...
  const rows = trace.map((row) => [String(row.step), String(row.line), row.statement, ...names.map((name) => String(row.variables[name] ?? ""))].join(" | "));
...
export async function runPseudocode(request: RuntimeRequest): Promise<RuntimeResult> {
  ...
  const state: ExecState = {
    ...
    diagnostics: [], inputs: [], inputIndex: 0, timeoutAt: started + Math.min(request.timeoutMs ?? 5000, 10000),
  };
  ...
  const input = (prompt?: string) => {
    if (prompt) state.output.push(String(evalExpr(prompt, state)));
    const raw = state.inputs[state.inputIndex++] ?? "0";
    return raw;
  };
  ...
  const executeRange = async (from: number, to: number): Promise<void> => {
    ...
    while (i < to && i < state.lines.length) {
      ...
      if (!line || /^(BEGIN|END|THEN|ELSE|ENDIF|END IF|ENDWHILE|END WHILE|ENDFOR|END FOR|UNTIL\b)/i.test(line)) { i += 1; continue; }
      try {
        ...
        if (ifMatch) {
          ...
          const split = elseIndex < 0 ? end : i + 1 + elseIndex;
          if (Boolean(evalExpr(ifMatch[1], state))) await executeRange(i + 1, split);
          else if (elseIndex >= 0) await executeRange(split + 1, end);
          i = end + 1; continue;
        }
        ...
        if (whileMatch) {
          ...
          let guard = 0;
          while (Boolean(evalExpr(whileMatch[1], state))) {
        ...
        const forMatch = line.match(/^FOR\s+(\w+)\s*(?:←|<-|=|TO)\s*(.+)$/i);
        if (forMatch) {
          const range = forMatch[2].split(/\s+TO\s+/i);
          if (range.length !== 2) { error("Use FOR counter <- start TO end.", i + 1); return; }
        ...
        if (inputMatch) { const name = inputMatch[1].trim(); state.vars[name] = valueOf(input(), state); }
        else if (/^OUTPUT\s+/.test(upper) || /^PRINT\s+/.test(upper)) state.output.push(String(evalExpr(line.replace(/^(OUTPUT|PRINT)\s+/i, ""), state)));
        else if (/^(SET\s+)?\w+\s*(←|<-|=)/.test(line)) {
          const match = line.match(/^(?:SET\s+)?(\w+)\s*(?:←|<-|=)\s*(.+)$/i);
          if (!match) { error("Invalid assignment syntax.", i + 1); return; }
          state.vars[match[1]] = evalExpr(match[2], state);
        } else if (/^(MOD|DIV)\s+/i.test(line)) { /* recognized operators are handled inside expressions */ }
        else { error(`Unsupported pseudocode statement: ${line}`, i + 1); return; }
        state.step += 1;
        state.trace.push({ step: state.step, line: i + 1, statement: line, variables: { ...state.vars } });
  ...
  const traceText = renderTrace(state.trace);
  const stdout = [...state.output, ...(traceText ? [traceText] : [])].join("\n");
  ...
  events.push({ type: failed ? "status" : "status", status: failed ? "failed" : "completed" });
  events.push({ type: "exit", code: failed ? 1 : 0 });
