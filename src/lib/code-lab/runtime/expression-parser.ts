...
type Token = { kind: "number"; value: number } | { kind: "string"; value: string } | { kind: "boolean"; value: boolean } | { kind: "identifier"; value: string } | { kind: "operator"; value: string } | { kind: "lparen" } | { kind: "rparen" } | { kind: "comma" } | { kind: "eof" };
...
function tokenize(input: string): Token[] {
  ...
  while (i < input.length) {
    ...
    if (rest[0] === "\"" || rest[0] === "'") { const quote = rest[0]; let j = 1; let value = ""; while (j < rest.length && rest[j] !== quote) { if (rest[j] === "\\" && j + 1 < rest.length) { value += rest[j + 1]; j += 2; } else value += rest[j++]; } if (j >= rest.length) throw new Error("Unterminated string literal."); tokens.push({ kind: "string", value }); i += j + 1; continue; }
    const number = rest.match(NUMBER); if (number) { tokens.push({ kind: "number", value: Number(number[0]) }); i += number[0].length; continue; }
    const identifier = rest.match(IDENTIFIER); if (identifier) { const word = identifier[0]; const upper = word.toUpperCase(); if (upper === "TRUE" || upper === "FALSE") tokens.push({ kind: "boolean", value: upper === "TRUE" }); else if (["AND", "OR", "NOT", "MOD", "DIV"].includes(upper)) tokens.push({ kind: "operator", value: upper }); else tokens.push({ kind: "identifier", value: word }); i += word.length; continue; }
    const two = rest.slice(0, 2); if (["<=", ">=", "!=", "<>", "=="].includes(two)) { tokens.push({ kind: "operator", value: two }); i += 2; continue; }
  ...
  tokens.push({ kind: "eof" }); return tokens;
}
function numeric(value: ExpressionValue | ExpressionValue[]) { if (typeof value === "number") return value; const n = Number(value); return Number.isFinite(n) ? n : 0; }
function truthy(value: ExpressionValue | ExpressionValue[]) { if (typeof value === "boolean") return value; if (Array.isArray(value)) return value.length > 0; return typeof value === "string" ? value.length > 0 : value !== 0; }
function equal(a: ExpressionValue | ExpressionValue[], b: ExpressionValue | ExpressionValue[]) { if (Array.isArray(a) || Array.isArray(b)) return JSON.stringify(a) === JSON.stringify(b); return a === b || (numeric(a) === numeric(b) && Number.isFinite(numeric(a)) && Number.isFinite(numeric(b))); }
...
export function evaluateExpression(input: string, context: ExpressionContext): ExpressionValue | ExpressionValue[] {
  ...
  if (notMatch) return !truthy(evaluateExpression(notMatch[1], context));
  const tokens = tokenize(trimmed.replace(/<>/g, "!=").replace(/\bDIV\b/gi, "DIV").replace(/\bMOD\b/gi, "MOD"));
  let position = 0;
  ...
  const primary = (): ExpressionValue | ExpressionValue[] => {
    ...
    if (token.kind === "identifier") {
      ...
      if (peek().kind === "operator" && (peek() as Extract<Token, { kind: "operator" }>).value === "[") { consume(); const index = numeric(orExpr()); const close = consume(); if (close.kind !== "operator" || close.value !== "]") throw new Error("Missing ] in array access."); return Array.isArray(value) ? value[Math.max(0, Math.trunc(index) - 1)] ?? "" : ""; }
      return value;
    }
    if (token.kind === "lparen") { const value = orExpr(); expect("rparen"); return value; }
    throw new Error("Expected a value.");
  };
  const unary = (): ExpressionValue | ExpressionValue[] => { if (peek().kind === "operator") { const op = (peek() as Extract<Token, { kind: "operator" }>).value; if (op === "!" || op === "+" || op === "-") { consume(); const value = unary(); if (op === "!") return !truthy(value); return op === "-" ? -numeric(value) : numeric(value); } } return primary(); };
