...
function tokenize(input: string): Token[] {
  ...
  while (i < input.length) {
    ...
    if (["+", "-", "*", "/", "%", "<", ">", "=", "!"].includes(rest[0])) { tokens.push({ kind: "operator", value: rest[0] }); i++; continue; }
...
export function evaluateExpression(input: string, context: ExpressionContext): ExpressionValue | ExpressionValue[] {
  ...
  const primary = (): ExpressionValue | ExpressionValue[] => {
    ...
    if (token.kind === "identifier") {
      ...
      if (upper === "LENGTH" && peek().kind === "lparen") { consume(); const value = orExpr(); expect("rparen"); return Array.isArray(value) || typeof value === "string" ? value.length : 0; }
      ...
      if (peek().kind === "operator" && (peek() as Extract<Token, { kind: "operator" }>).value === "[") {
        consume();
        const index = numeric(unary());
        const close = consume();
        if (close.kind !== "operator" || close.value !== "]") throw new Error("Missing ] in array access.");
  ...
  const unary = (): ExpressionValue | ExpressionValue[] => {
    if (peek().kind === "operator") {
      ...
      if (op === "NOT" || op === "!" || op === "+" || op === "-") {
        consume(); const value = unary();
        if (op === "NOT" || op === "!") return !truthy(value);
        return op === "-" ? -numeric(value) : numeric(value);
      }
    }
    return primary();
  };
  ...
  const result = orExpr();
  if (peek().kind !== "eof") throw new Error("Unexpected tokens after expression.");
  return result;
}
