...
describe("browser polyglot runtimes", () => {
  it("transpiles and executes TypeScript without treating it as JavaScript source", async () => {
    const result = await runBrowserTypeScript({
      id: "typescript-test",
      language: "typescript",
      code: "const value: number = 21 * 2; console.log(value);",
      entryFile: "main.ts",
      timeoutMs: 5000,
    });
  ...
  it("executes SQL against an isolated in-memory database", async () => {
    const result = await runBrowserSql({
      id: "sql-test",
      language: "sql",
      code: "CREATE TABLE students (name TEXT, mark INTEGER); INSERT INTO students VALUES ('A', 80), ('B', 90); SELECT AVG(mark) AS average FROM students;",
      entryFile: "main.sql",
      timeoutMs: 5000,
    });
    expect(result.exitCode).toBe(0);
