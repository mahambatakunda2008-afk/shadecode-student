...
describe("Comp Lab algorithm assessment", () => {
  ...
  it("partitions visible and hidden assessment cases", () => {
    const exercise = getAlgorithmExercise("largest-three")!;
    expect(getVisibleTestCases(exercise)).toHaveLength(3);
    expect(getHiddenTestCases(exercise)).toHaveLength(2);
    expect(getAssessmentTestCases(exercise).find(x => x.id === "hidden-negative")?.expectedOutput).toBe("-3");
  });
  it("weights hidden tests without exposing their inputs", () => {
  ...
  it("records compact Learning Graph-ready evidence", () => {
