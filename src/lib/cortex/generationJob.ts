...
function write(jobs: GenerationJob<unknown, unknown>[]) {
  if (!isBrowser()) return;
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(jobs.slice(-MAX_JOBS))); listeners.forEach(listener => listener()); } catch {}
}
function notifyStorageChange() { if (isBrowser()) window.dispatchEvent(new CustomEvent("shadecode:cortex:jobs")); }
...
export function createGenerationJob<TRequest>(kind: GenerationJob["kind"], request: TRequest): GenerationJob<TRequest> {
  ...
  write([...read(), job as GenerationJob<unknown, unknown>]); notifyStorageChange(); return job;
}
export function updateGenerationJob<TRequest = unknown, TResult = unknown>(id: string, patch: Partial<GenerationJob<TRequest, TResult>>) {
  const jobs = read(); const index = jobs.findIndex(job => job.id === id); if (index < 0) return null;
  const current = jobs[index];
  jobs[index] = { ...current, ...patch, progress: typeof patch.progress === "number" ? Math.max(0, Math.min(100, Math.round(patch.progress))) : current.progress, updatedAt: Date.now() } as GenerationJob<unknown, unknown>;
  write(jobs); notifyStorageChange(); return jobs[index] as GenerationJob<TRequest, TResult>;
}
...
export function subscribeGenerationJobs(listener: () => void) {
  ...
  return () => { listeners.delete(listener); if (isBrowser()) { window.removeEventListener("storage", listener); window.removeEventListener("shadecode:cortex:jobs", listener); } };
}

export function markInterruptedJobsForRetry(staleAfterMs = DEFAULT_STALE_AFTER_MS) {
  const jobs = read(); const now = Date.now(); let changed = false;
  const next = jobs.map(job => {
    const stale = now - job.updatedAt >= staleAfterMs;
    if (!INTERRUPTIBLE_STATUSES.has(job.status) || !stale) return job;
    changed = true;
    return { ...job, status: "queued" as const, progress: Math.min(job.progress, 90), retryCount: job.retryCount + 1, updatedAt: now, error: "Previous generation run stopped. Cortex will resume this request." };
  });
