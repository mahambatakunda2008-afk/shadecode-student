/** Cortex lesson generation with structured coverage, curriculum-aware planning, diagram-aware content and safe failure handling. */
import { callAI } from "@/lib/ai";
import { getMemory } from "./memory";
import { awardXPBySource } from "@/lib/xp/manager";
import type { DiagramSpec } from "@/lib/learning/content";
import { buildLessonBlueprint, renderLessonBlueprint } from "./lessonBlueprint";
...
function withTimeout<T>(promise: Promise<T>, timeoutMs: number, fallback: T): Promise<T> { return new Promise(resolve => { const timer = setTimeout(() => resolve(fallback), timeoutMs); promise.then(value => { clearTimeout(timer); resolve(value); }).catch(() => { clearTimeout(timer); resolve(fallback); }); }); }

function normalizeCoverage(raw: unknown): LessonCoverage { const result = emptyCoverage(); if (!raw || typeof raw !== "object") return result; for (const key of Object.keys(result) as (keyof LessonCoverage)[]) { const value = Number((raw as Record<string, unknown>)[key]); result[key] = Number.isFinite(value) ? Math.max(0, Math.min(1, value)) : 0; } return result; }

function normalizeLesson(subject: string, raw: unknown): GeneratedLesson | null {
  if (!raw || typeof raw !== "object") return null;
  const x = raw as Record<string, unknown>;
  const sections = Array.isArray(x.sections) ? x.sections.map((s) => { if (!s || typeof s !== "object") return null; const v = s as Record<string, unknown>; const allowed = ["example", "definition", "tip", "misconception", "exam", "curiosity"] as const; const type = allowed.includes(v.type as typeof allowed[number]) ? v.type as LessonSection["type"] : "explanation"; return typeof v.heading === "string" && typeof v.content === "string" ? { heading: v.heading.slice(0, 200), content: v.content.slice(0, 7000), type } : null; }).filter(Boolean).slice(0, 14) as LessonSection[] : [];
  const diagrams = Array.isArray(x.diagrams) ? x.diagrams.map((d) => { if (!d || typeof d !== "object") return null; const v = d as Record<string, unknown>; return typeof v.title === "string" && typeof v.description === "string" ? { title: v.title.slice(0, 200), description: v.description.slice(0, 2000), spec: v.spec as DiagramSpec | undefined } : null; }).filter(Boolean).slice(0, 6) as LessonDiagram[] : [];
  const practiceQuestions = Array.isArray(x.practiceQuestions) ? x.practiceQuestions.map((q) => { if (!q || typeof q !== "object") return null; const v = q as Record<string, unknown>; const difficulty = v.difficulty === "easy" || v.difficulty === "hard" ? v.difficulty : "medium"; return typeof v.question === "string" && typeof v.correctAnswer === "string" && typeof v.explanation === "string" ? { question: v.question.slice(0, 1800), options: Array.isArray(v.options) ? v.options.filter((o): o is string => typeof o === "string").slice(0, 6) : undefined, correctAnswer: v.correctAnswer.slice(0, 800), explanation: v.explanation.slice(0, 3000), difficulty, skill: typeof v.skill === "string" ? v.skill.slice(0, 120) : undefined, diagram: v.diagram as DiagramSpec | undefined } : null; }).filter(Boolean).slice(0, 12) as PracticeQuestion[] : [];
  if (typeof x.title !== "string" || typeof x.summary !== "string" || sections.length < 5 || practiceQuestions.length < 3) return null;
  const difficulty = x.difficulty === "easy" || x.difficulty === "hard" ? x.difficulty : "medium"; const minutes = Number(x.estimatedMinutes);
...
export async function generateLesson(subject: string, topic: string, userId: string, curriculum?: CurriculumResolution): Promise<GeneratedLesson | null> {
  try {
    ...
    const scope = curriculum ? renderLessonBlueprint(buildLessonBlueprint(curriculum)) : `Subject: ${subject}\nTopic: ${topic}`;
    ...
    const jsonMatch = response.match(/\{[^]*\}/); if (!jsonMatch) return null;
    const lesson = normalizeLesson(subject, JSON.parse(jsonMatch[0])); if (!lesson) return null;
    void withTimeout(awardXPBySource(userId, "lesson_generation", { difficulty }), XP_BUDGET_MS, undefined); return lesson;
  } catch (error) { console.error("[LessonGenerator] Failed:", error); return null; }
}
