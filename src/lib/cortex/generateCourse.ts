...
export async function generateCourseForUser(userToken: string, params: { topic: string; goal: string; level?: string }) {
  ...
  const lessonsToInsert = parsed.lessons.slice(0, 50).map((l: any) => ({
    user_id: user.id,
  ...
  for (const l of parsed.lessons) {
    ...
    for (const p of prereqs) {
      const pid = titleToId.get(p.toString().slice(0,255));
  ...
  return { title: parsed.title ?? subjName, lessonsInserted: (inserted.data ?? []).length };
}
