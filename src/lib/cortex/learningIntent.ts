...
type Rule = { intent: LearningIntent; patterns: RegExp[]; reason: string };

// Prompt rules are deliberately specific. Broad words such as “test” or “wrong”
// are avoided because they collide with otherwise valid learning requests.
const PROMPT_RULES: Rule[] = [
  { intent: "guided-solve", patterns: [/help me solve/i, /how do i solve/i, /walk me through/i, /show me how to solve/i, /stuck on/i, /guide me through/i], reason: "The learner wants guided problem solving rather than a standalone lesson." },
  { intent: "practice", patterns: [/give me\s+\d*\s*(?:questions?|problems?)/i, /practice/i, /quiz me/i, /quiz/i, /questions? on/i, /test me/i, /drill me/i, /drill/i], reason: "The learner explicitly asks to practise or be tested." },
  { intent: "remediate", patterns: [/keep getting/i, /always get.*wrong/i, /don't understand/i, /do not understand/i, /struggling with/i, /weak at/i, /confused about/i, /misunderstand/i, /can't understand/i, /cannot understand/i], reason: "The learner signals confusion, repeated errors, or a weak area." },
  { intent: "exam-prep", patterns: [/exam/i, /past paper/i, /mark scheme/i, /revision/i, /revise/i, /assessment/i, /mock paper/i, /exam-style/i], reason: "The request is framed around assessment preparation or revision." },
  { intent: "deep-dive", patterns: [/why does/i, /why is/i, /derive/i, /in depth/i, /deep dive/i, /first principles/i, /prove/i, /derive from/i], reason: "The request asks for causal, foundational, or rigorous explanation." },
  { intent: "from-scratch", patterns: [/from scratch/i, /know nothing/i, /beginner/i, /start from basics/i, /teach me the basics/i, /start from the beginning/i], reason: "The learner explicitly asks to begin from foundational knowledge." },
  { intent: "review", patterns: [/quickly/i, /quick review/i, /recap/i, /summary/i, /summarise/i, /summarize/i, /refresh me/i, /refresher/i], reason: "The learner requests a concise review rather than a full lesson." },
];
...
function findRule(input: string, rules: Rule[]) {
...
export function resolveLearningIntent(prompt: string, goal = ""): LearningIntentResult {
  ...
  return { intent: "learn", confidence: cleanPrompt ? "medium" : "low", reason: "No specialised learning mode was requested, so Cortex will teach the requested topic directly." };
}
