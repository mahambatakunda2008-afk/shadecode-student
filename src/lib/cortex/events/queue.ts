...
export function subscribeToCortexEvents(listener: (event: CortexEvent) => void) { if (!isBrowser()) return () => undefined; const handler = (event: Event) => { const customEvent = event as CustomEvent<CortexEvent>; if (customEvent.detail) listener(customEvent.detail); }; window.addEventListener(BROWSER_EVENT, handler); return () => window.removeEventListener(BROWSER_EVENT, handler); }
