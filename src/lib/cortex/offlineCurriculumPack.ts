...
function normalizeKnowledge(value: unknown): OfflineCurriculumKnowledgeItem | null {
  ...
  return {
    ...
    topicCode: typeof item.topicCode === "string" ? item.topicCode : undefined,
...
export function readOfflineCurriculumPack(subject: string, board?: string, level?: string): OfflineCurriculumPack | null {
  ...
  try {
    ...
    if (!raw) return null;
    const value = JSON.parse(raw) as Partial<OfflineCurriculumPack>;
    ...
    return {
      ...
      syllabusVersion: value.syllabusVersion,
...
export function buildOfflineCurriculumScope(pack: OfflineCurriculumPack | null, topic: string) {
  ...
  const matchingKnowledge = pack.knowledge
    .map(item => ({ item, score: topicTerms.reduce((n, term) => n + (`${item.kind} ${item.title} ${item.content} ${item.topicCode || ""}`.toLowerCase().includes(term) ? 1 : 0), 0) }))
    .filter(x => x.score > 0)
    .sort((a, b) => b.score - a.score)
    .slice(0, 120)
    .map(x => `- ${x.item.kind}: ${x.item.code ? `[${x.item.code}] ` : ""}${x.item.title}${x.item.content ? ` | ${x.item.content.slice(0, 600)}` : ""}`)
    .join("\n");
