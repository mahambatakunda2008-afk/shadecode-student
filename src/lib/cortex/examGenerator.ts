...
function normalizeQuestion(q: Partial<ExamQuestion>, index: number, difficulty: string): ExamQuestion {
  ...
  const options = requestedType === "multiple_choice" ? (Array.isArray(q.options) ? q.options.filter((o): o is string => typeof o === "string" && o.trim().length > 0).slice(0, 4) : []) : undefined;
  const type = requestedType === "multiple_choice" && options?.length !== 4 ? "short_answer" : requestedType;
  return { id: `q_${index + 1}_${Date.now()}`, type, question: q.question!.trim().slice(0, 4000), options: type === "multiple_choice" ? options : undefined, marks: Number.isFinite(Number(q.marks)) ? Math.max(1, Math.min(20, Math.round(Number(q.marks)))) : 1, topic: q.topic!.trim().slice(0, 255), difficulty: normalizedDifficulty, modelAnswer: typeof q.modelAnswer === "string" ? q.modelAnswer.slice(0, 4000) : undefined, markingCriteria: typeof q.markingCriteria === "string" ? q.markingCriteria.slice(0, 2000) : undefined };
}
function isExamPayload(value: unknown): value is { title?: unknown; questions: unknown[]; durationMinutes?: unknown; topics?: unknown } { return !!value && typeof value === "object" && Array.isArray((value as { questions?: unknown }).questions); }
  ...
  try {
    ...
    if (response) {
      ...
      if (rawQuestions.length >= Math.max(1, Math.ceil(safeCount * 0.6))) {
        const questions = rawQuestions.slice(0, safeCount).map((q, i) => normalizeQuestion(q, i, difficulty));
        if (questions.length) return { subject, title: typeof parsed?.title === "string" && parsed.title.trim() ? parsed.title.trim().slice(0, 255) : `${subject} Practice Exam`, questions, totalMarks: questions.reduce((s, q) => s + q.marks, 0), durationMinutes: Math.max(5, Math.min(240, Math.round(Number(parsed?.durationMinutes) || safeCount * 3))), difficulty, topics: Array.isArray(parsed?.topics) ? parsed.topics.filter((t): t is string => typeof t === "string").slice(0, 20) : topics };
      }
    }
