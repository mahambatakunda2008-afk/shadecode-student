...
function normalizeQuestion(q: Partial<ExamQuestion>, index: number, subject: string, difficulty: string): ExamQuestion {
  ...
  return { id: `q_${index + 1}_${Date.now()}`, type: options && options.length === 4 ? type : type === "multiple_choice" ? "short_answer" : type, question: q.question!.trim().slice(0, 4000), options: options && options.length === 4 ? options : undefined, marks: Number.isFinite(q.marks) ? Math.max(1, Math.min(20, Math.round(q.marks as number))) : 1, topic: q.topic!.trim().slice(0, 255), difficulty: normalizedDifficulty, modelAnswer: typeof q.modelAnswer === "string" ? q.modelAnswer.slice(0, 4000) : undefined, markingCriteria: typeof q.markingCriteria === "string" ? q.markingCriteria.slice(0, 2000) : undefined };
}
  ...
  try {
    ...
    if (response) {
      ...
      if (rawQuestions.length >= Math.max(1, Math.ceil(safeCount * 0.6))) {
        ...
        if (questions.length) {
          const title = typeof parsed?.title === "string" && Boolean(parsed.title.trim()) ? parsed.title.trim().slice(0, 255) : `${subject} Practice Exam`;
          return { subject, title, questions, totalMarks: questions.reduce((s, q) => s + q.marks, 0), durationMinutes: Math.max(5, Math.round(Number(parsed?.durationMinutes) || safeCount * 3)), difficulty, topics: Array.isArray(parsed?.topics) ? parsed.topics.filter((t): t is string => typeof t === "string").slice(0, 20) : topics };
        }
      }
    }
