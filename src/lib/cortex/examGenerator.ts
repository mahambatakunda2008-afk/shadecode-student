...
- Mathematics: actual expressions, equations, numerical values, graphs, proof, modelling, or multi-step calculations.
- Physics: physical quantities, units, equations, calculations, experiments/data, graphs, explanations, and realistic contexts.
- Chemistry: equations, structures, calculations, observations, trends, mechanisms, bonding, energetics, equilibria, or practical interpretation.
- Computer Science: algorithms, pseudocode, trace tables, data structures, computational thinking, complexity, architecture, databases, networking, or programming tasks.
- Biology: processes, data, experimental design, calculations, graphs, application and evaluation.
- Economics/Business/Accounting/Geography/History/English: realistic scenarios, source/data interpretation, structured analysis, evaluation, and evidence where appropriate.

QUESTION MIX
Create a deliberate mix of question types. MCQs must have exactly four plausible, distinct options with one clearly correct answer. For structured questions use parts (a), (b), (c) where useful. Keep the full question in one string.
...
Do a silent examiner-quality check before returning JSON. Remove weak, generic, duplicated, under-specified, outside-topic, or mark-mismatched questions before returning them.`;

const MEMORY_BUDGET_MS = 2500;
const AI_BUDGET_MS = 42000;
...
function isQuestion(value: unknown): value is Partial<ExamQuestion> {
  ...
  return typeof q.question === "string" && q.question.trim().length >= 24 && Number.isFinite(Number(q.marks));
}
...
function topicMatches(requestedTopic: string, questionTopic: string, questionText: string): boolean {
  ...
  if (!actual) return text.includes(wanted);
...
function normalizeQuestion(q: Partial<ExamQuestion>, index: number, difficulty: string, requestedTopic: string): ExamQuestion | null {
  ...
  const questionTopic = typeof q.topic === "string" ? cleanTopic(q.topic.trim()) : "";
  if (!topicMatches(requestedTopic, questionTopic, q.question!)) return null;
...
export async function generateExam(subject: string, topics: string[], difficulty: string, questionCount: number, userId: string): Promise<GeneratedExam | null> {
  ...
  try {
    ...
    const response = await withTimeout(callAI(prompt, 6000, {
      ...
      maxChainMs: AI_BUDGET_MS,
      perProviderMaxMs: 9000,
    ...
    if (!parsed) {
      console.warn(`[ExamGenerator:${GENERATOR_VERSION}] AI returned invalid JSON after repair.`);
      return null;
    }
