...
function attachSavedLearningContext(input: LessonGenerationInput): LessonGenerationInput {
  if (!isBrowser()) return input;
  try {
    const raw = localStorage.getItem(CONTEXT_KEY);
    if (!raw) return input;
    const saved = JSON.parse(raw) as { level?: unknown; examBoard?: unknown; goal?: unknown };
    const level = typeof saved.level === "string" && saved.level.trim() ? saved.level.trim() : input.level;
    const examBoard = typeof saved.examBoard === "string" && saved.examBoard.trim() ? saved.examBoard.trim() : input.examBoard;
    const selectedGoal = typeof saved.goal === "string" && saved.goal.trim() ? saved.goal.trim() : "";
    const goal = selectedGoal ? `${selectedGoal}. ${input.goal}` : input.goal;
    return { ...input, level, examBoard, goal };
  } catch {
    return input;
  }
}
...
export function parseLocalLesson(text: string, request: LessonGenerationInput): LessonGenerationResult {
  const normalized = text.replace(/```(?:markdown|md)?/gi, "").replace(/```/g, "").replace(/^\s*(?:assistant|cortex)\s*:\s*/i, "").trim();
  ...
  const sections = body.split(/\n(?=##\s+|###\s+|\d+[.)]\s+|\*\*[^*]+\*\*\s*$)/g).map(section => section.trim()).filter(Boolean);
  const blocks = (sections.length ? sections : [body]).map((section, index) => {
    ...
    return { type: index === 0 ? "explanation" : "section", title: sectionTitle || (index === 0 ? "Lesson" : `Part ${index + 1}`), content };
...
async function persistLesson(result: LessonGenerationResult, request: LessonGenerationInput) {
  ...
  await offlineStorage.saveLesson({ id: result.id, title: result.title, subject: request.subject, description: `A complete ${request.difficulty} lesson on ${request.prompt}`, blocks: result.blocks, difficulty: request.difficulty, progress: 0, completed: false, downloadedAt: now, lastSyncedAt: now, size: JSON.stringify(result).length });
}
...
async function runLocalJob(job: GenerationJob<LessonGenerationInput>) {
  ...
  if (!isLocalCortexPrepared()) {
    if (!isBrowser() || !navigator.onLine) throw new Error("Local Cortex has not been prepared on this device. Connect once to prepare the local teaching model.");
    ...
    await runtime.warm(progress => updateGenerationJob(job.id, { status: "warming", engine: "local", progress: Math.min(30, Math.max(2, Math.round(progress * 0.3))) }));
  }
  ...
  if (!quality.passed) throw new Error(`Local Cortex draft did not meet the lesson quality bar (${result.blocks.length} sections, ${quality.signalCount}/7 quality signals).`);
...
async function runCloudJob(job: GenerationJob<LessonGenerationInput>, token: string) {
  ...
  const response = await fetch("/api/learn/generate", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ type: "lesson", subject: job.request.subject, topic: job.request.prompt, prompt: job.request.prompt, difficulty: job.request.difficulty, goal: job.request.goal, level: job.request.level, examBoard: job.request.examBoard }), cache: "no-store", keepalive: true });
...
async function runJob(job: GenerationJob<LessonGenerationInput>, initialToken: string | null) {
  ...
  try {
    try { return await runLocalJob(job); }
    catch (localError) {
    ...
    updateGenerationJob(job.id, { status: offlineNow ? "queued" : "failed", progress: offlineNow ? Math.min(job.progress, 20) : job.progress, error: offlineNow ? "Local Cortex is not ready yet. Your request is safely queued on this device." : errorMessage(error) });
...
export function queueLessonGeneration(input: LessonGenerationInput) {
  const job = createGenerationJob("lesson", attachSavedLearningContext(input));
...
export async function startLessonGeneration(input: LessonGenerationInput, token: string | null) {
  ...
  if (isBrowser()) { void runJob(job, token); return getGenerationJobs().find(item => item.id === job.id) ?? job; }
  return job;
}