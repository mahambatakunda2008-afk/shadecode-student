...
function errorMessage(value: unknown) {
  return value instanceof Error ? value.message : "Lesson generation failed.";
}
...
async function runJob(job: GenerationJob<LessonGenerationInput>, token: string) {
  ...
  running = true;
  saveActiveId(job.id);
  updateGenerationJob(job.id, { status: "generating", progress: Math.max(8, job.progress) });
  ...
  try {
    ...
    await offlineStorage.saveLesson({
      ...
      title: result.title,
      subject: job.request.subject,
      description: `A complete ${job.request.difficulty} lesson on ${job.request.prompt}`,
    ...
    updateGenerationJob(job.id, {
      ...
      error: offlineNow ? "Waiting for a connection. Your request is safely queued on this device." : message,
    });
    if (offlineNow) saveActiveId(job.id);
    else if (getActiveId() === job.id) saveActiveId(null);
    return getGenerationJobs().find((item) => item.id === job.id) ?? job;
  } finally {
    running = false;
  }
...
export function queueLessonGeneration(input: LessonGenerationInput) {
  ...
  if (isBrowser() && !navigator.onLine) {
    updateGenerationJob(job.id, {
      status: "queued",
      progress: 0,
      error: "Waiting for a connection. Your request is safely queued on this device.",
    });
  }
  return job;
}
