...
function parseLocalModelLesson(raw: string, job: GenerationJob<LessonGenerationInput>): LessonGenerationResult | null {
  try {
    ...
    // Apply a lightweight local quality gate before accepting model output.
    // The server remains the authoritative curriculum/quality gate for cloud lessons.
    const types = new Set(blocks.map(block => String(block.type).toLowerCase()));
    const hasBackbone = types.has("objective") && (types.has("concept") || types.has("definition")) && types.has("example") && types.has("checkpoint") && types.has("summary");
    if (!hasBackbone) return null;
    const wallOfText = blocks.filter(block => {
      const content = String(block.content);
      return content.length > 1100 && !content.includes("\n") && !/[.!?]\s+[A-Z0-9]/.test(content);
    }).length;
    if (wallOfText > 1) return null;
...
async function runJob(job: GenerationJob<LessonGenerationInput>, token: string) {
  ...
  try {
    ...
    // Local-first. This is deliberately attempted even while offline because a local
    // Ollama server does not require internet access.
    ...
    // No local model: if there is no network, open the deterministic offline study
    // experience immediately rather than waiting for a cloud request that cannot work.
    ...
    if (isBrowser() && hasLocalLessonFallback(job.request.subject, job.request.prompt)) {
