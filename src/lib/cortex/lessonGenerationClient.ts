...
async function saveLocalResult(job: GenerationJob<LessonGenerationInput>, generated?: LessonGenerationResult) {
  ...
  await offlineStorage.saveLesson({ id: result.id, title: result.title, subject: job.request.subject, description: generated?.localModel ? `Locally generated Cortex lesson for ${job.request.prompt}` : `Offline lesson for ${job.request.prompt}`, blocks: result.blocks, difficulty: job.request.difficulty, progress: 0, completed: false, downloadedAt: now, lastSyncedAt: now, size: JSON.stringify(result).length });
...
async function tryLocalModel(job: GenerationJob<LessonGenerationInput>): Promise<LessonGenerationResult | null> {
  ...
  try {
    ...
    const prompt = `You are Shadecode Student local Cortex. Return ONLY JSON. Build a real teaching lesson, not a study checklist or placeholder. Subject: ${job.request.subject}. Level: ${job.request.level || "not specified"}. Exam board: ${job.request.examBoard || "not specified"}. Difficulty: ${job.request.difficulty}. Goal: ${job.request.goal}. Topic: ${job.request.prompt}.${scope}${cachedContext}\nUse short, organized learning units. Teach the concept, definitions and formulas, then show worked reasoning, checkpoint questions, exam transfer and progressive practice. Never write a wall of text. Never tell the student to look up the core lesson instead of teaching it when the information is present in the supplied curriculum context. Return 10-16 blocks using objective, prior, concept, definition, formula, example, checkpoint, misconception, exam, application, mistake, practice, summary, tip. Worked examples must use Given:, Method:, Step 1:, Step 2:, Answer:. Checkpoints must use Question: and Think: or Answer:. Exam transfer must use Question:, Approach:, Examiner looks for:. JSON: {"title":"...","blocks":[{"type":"...","title":"...","content":"..."}]}`;
