...
export class OfflineSync {
  ...
  private async bridgeLocalOperations(): Promise<void> {
    ...
    const deviceId = await localFirstStore.deviceId();
  ...
  private operationToMutation(operation: LocalOperation, deviceId: string): Omit<OfflineMutation, "id" | "createdAt" | "attempts" | "ownerId"> {
    ...
    return {
      ...
      store,
      payload: { ...(payload as Record<string, unknown>), id: operation.entityId, user_id: operation.userId },
      clientVersion: operation.lamport,
      baseVersion: operation.lamport > 0 ? operation.lamport - 1 : 0,
      deviceId,
    };
  }
  ...
  private async syncMutations(): Promise<void> {
    ...
    for (const mutation of mutations) {
      try {
        ...
        if (!/^[a-zA-Z0-9_]+$/.test(table) || !mutationQueue) throw new Error(`Invalid offline mutation store: ${table}`);
        if (mutation.operation === "delete") {
          ...
          const { error } = await supabase.from(table).upsert({ ...payload, user_id: user.id }, { onConflict: "id" });
          if (error) throw error;
        }
        const entity = table === "tasks" ? "task" : table === "subjects" ? "subject" : null;
        const entityId = typeof payload.id === "string" ? payload.id : null;
        if (entity && entityId) await localFirstStore.acknowledgeEntityOperations(user.id, entity, entityId, mutation.clientVersion);
  ...
  private async syncProgress(): Promise<void> {
    ...
    if (!auth) return;
    const { supabase, user } = auth;
    for (const progress of await localFirstStore.listPendingProgress(user.id)) {
      try {
  ...
  async saveTaskLocally(taskId: string): Promise<void> {
    ...
    await localFirstStore.hydrate({ id: task.id, entity: "task", userId: auth.user.id, payload: { id: task.id, userId: auth.user.id, subject_id: task.subject_id, title: task.title, completed: task.completed, lastUpdated: task.updated_at ?? new Date().toISOString(), synced: true }, updatedAt: Date.parse(task.updated_at ?? "") || Date.now(), deviceId: "server", version: 0 });
  }
  ...
  async saveProgressLocally(lessonId: string, userId: string): Promise<void> {
    ...
    await localFirstStore.hydrateProgress({ lessonId: lesson.id, userId: auth.user.id, completed: lesson.progress === 100, progress: lesson.progress, lastUpdated: lesson.updated_at ?? new Date().toISOString() }, Date.parse(lesson.updated_at ?? "") || Date.now());
  }
  ...
  async getSubjects(userId: string) {
    ...
    if (error) return [];
    for (const subject of subjects || []) await localFirstStore.hydrate({ id: subject.id, entity: "subject", userId: auth.user.id, payload: { id: subject.id, userId: auth.user.id, name: subject.name, lastUpdated: subject.updated_at ?? new Date().toISOString(), synced: true }, updatedAt: Date.parse(subject.updated_at ?? "") || Date.now(), deviceId: "server", version: 0 });
  ...
  async getProgress(lessonId: string, userId: string) {
    ...
    const progress = { lessonId: lesson.id, userId: auth.user.id, completed: lesson.progress === 100, progress: lesson.progress, lastUpdated: lesson.updated_at ?? new Date().toISOString() } as const;
