...
/*
 * Durable Cortex events live in Supabase. The in-memory store remains only as
 * a compatibility fallback for synchronous callers. New server-side flows
 * should use emitCortexEventDurable so events survive deploys and restarts.
 */
...
export async function emitCortexEventDurable(
  input: CortexEventInput
): Promise<CortexEvent> {
  const event = toEvent(input);
...
function snapshotFromEvents(userEvents: CortexEvent[]): CortexSnapshot {
  ...
  return {
    streak: 0,
    level: Math.max(1, Math.floor((completed.length * 10) / 100) + 1),
    xp: completed.length * 10,
    totalTasks: tasks.length,
    completedTasks: completed.length,
    pendingTasks: Math.max(0, tasks.length - completed.length),
    subjects: [],
    recentTaskTitles: [],
  };
}
...
export function buildSnapshot(userId: string): CortexSnapshot {
  return snapshotFromEvents(eventStore.filter((e) => e.userId === userId));
}
...
export async function buildDurableSnapshot(
  ...
  if (error) {
    console.error("Failed to load Cortex events:", error);
    return buildSnapshot(userId);
  }

  const events: CortexEvent[] = (data ?? []).map((row) => ({
    id: row.id,
    userId: row.user_id,
    type: row.type,
    source: row.source,
...
export async function recordCortexInsight(
  ...
  try {
    ...
    if (error) console.error("Error inserting insight:", error);
  } catch (err) {
