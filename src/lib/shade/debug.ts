...
export class ShadeDebugSession {
  ...
  getState(): ShadeDebugState {
    return {
      cursor: this.cursor,
      event: this.trace.events[this.cursor] ?? null,
      stopped: this.cursor >= this.trace.events.length - 1,
      reason: this.cursor < 0 ? "start" : this.cursor >= this.trace.events.length - 1 ? "end" : "breakpoint",
    };
  }
  continue(): ShadeDebugState {
    for (let index = this.cursor + 1; index < this.trace.events.length; index += 1) {
      this.cursor = index;
      if (this.breakpoints.has(this.trace.events[index].location.line)) break;
    }
    return this.getState();
  }
  stepOver(): ShadeDebugState {
    if (this.cursor >= this.trace.events.length - 1) return this.getState();
    const current = this.trace.events[this.cursor];
    const targetDepth = current?.callDepth ?? 0;
    const targetFrame = current?.frameId;
    for (let index = this.cursor + 1; index < this.trace.events.length; index += 1) {
      const next = this.trace.events[index];
      const nextDepth = next.callDepth ?? 0;
      const sameFrame = targetFrame === undefined || next.frameId === undefined || next.frameId === targetFrame;
      if (nextDepth <= targetDepth && sameFrame) {
        this.cursor = index;
        return this.getState();
      }
    }
    this.cursor = this.trace.events.length - 1;
    return this.getState();
  }
