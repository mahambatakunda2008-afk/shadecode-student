...
export type ShadeIROp = "const" | "load" | "store" | "binary" | "call" | "show" | "input" | "jump_if_false" | "jump" | "label" | "return";
...
export function lowerShadeToIR(program: ShadeProgram): ShadeIR {
  ...
  const statement = (node: ShadeStatement) => {
    switch (node.type) {
      case "assignment": expression(node.expression, node.line); emit("store", [node.name], node.line, node.expression.location?.column); break;
      case "show": expression(node.expression, node.line); emit("show", [], node.line, node.expression.location?.column); break;
      case "input": if (node.prompt) expression(node.prompt, node.line); emit("input", [node.name], node.line, node.prompt?.location?.column); break;
      case "if": { expression(node.condition, node.line); const elseLabel = `L${nextLabel++}`; const endLabel = `L${nextLabel++}`; emit("jump_if_false", [elseLabel], node.line, node.condition.location?.column); node.thenBody.forEach(statement); emit("jump", [endLabel], node.line); labels[elseLabel] = instructions.length; emit("label", [elseLabel], node.line); node.elseBody.forEach(statement); labels[endLabel] = instructions.length; emit("label", [endLabel], node.line); break; }
      case "while": { const startLabel = label(node.line, node.condition.location?.column); expression(node.condition, node.line); const endLabel = `L${nextLabel++}`; emit("jump_if_false", [endLabel], node.line, node.condition.location?.column); node.body.forEach(statement); emit("jump", [startLabel], node.line); labels[endLabel] = instructions.length; emit("label", [endLabel], node.line); break; }
      case "for": expression(node.iterable, node.line); emit("call", ["iterate", node.name], node.line, node.iterable.location?.column); node.body.forEach(statement); break;
      case "function": emit("label", [`fn:${node.name}`, ...node.params], node.line); node.body.forEach(statement); break;
      case "return": if (node.expression) expression(node.expression, node.line); emit("return", [], node.line, node.expression?.location?.column); break;
