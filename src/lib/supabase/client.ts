...
/**
 * Bound browser-side Supabase requests so a stalled network connection cannot
 * leave auth/data callers waiting forever. Callers can still supply their own
 * AbortSignal; both cancellation paths are respected.
 */
function fetchWithTimeout(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  ...
  const timeout = window.setTimeout(() => controller.abort(new DOMException('Supabase request timed out', 'TimeoutError')), SUPABASE_BROWSER_TIMEOUT_MS);
...
export function createClient() {
  ...
  // Browser client. Bound fetch prevents a stalled Supabase request from
  // freezing dashboard/auth flows indefinitely.
