...
export async function getLearnerContextForUser(supabase: SupabaseClient, user: User): Promise<LearnerContext | null> {
  ...
  if (profileError || academicError || subjectsError) throw new Error('Could not load academic profile.');
  ...
  const subjects = [...new Set([...(subjectRows ?? []).map(row => row.name), ...strings(profile?.subject_interests), ...strings(profile?.enrolled_courses), ...strings(academic?.courses)])];

  if (!profile?.onboarding_completed || !stage || !board || !qualification || !subjects.length) return null;
  ...
  return {
    ...
    board,
    qualification,
    syllabusCode: typeof metadata.syllabus_code === 'string' ? metadata.syllabus_code.trim() : undefined,
    syllabusYear: typeof metadata.syllabus_year === 'string' ? metadata.syllabus_year.trim() : undefined,
