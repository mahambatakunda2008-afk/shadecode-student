...
describe("pointer geometry", () => {
  it("moves a bounds by pointer delta", () => expect(moveBoundsFromPointer(bounds, { x: 5, y: 5 }, { x: 15, y: 0 })).toEqual({ x: 20, y: 15, width: 30, height: 40, rotation: 0 }));
  ...
  it("creates a normalized lasso rectangle", () => expect(boundsForRect({ x: 50, y: 40 }, { x: 10, y: 20 })).toEqual({ x: 10, y: 20, width: 40, height: 20, rotation: 0 }));
});
