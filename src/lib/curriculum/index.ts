...
export function computeCurriculumState(
  ...
  (function removeIntraCycleEdges() {
    ...
    for (const id of ids) if (indices[id] === undefined) strongconnect(id);
  ...
  function isUnlocked(lessonId: string) {
    ...
    return reqs.length === 0 || reqs.every((rid) => completedSet.has(rid));
  }
  ...
  if (!recommendedNextLesson) {
    ...
    if (incomplete.length > 0) {
      const ranked = incomplete.map((l) => {
        const reqs = prereqMap[l.id] ?? [];
        return { lesson: l, unmet: reqs.filter((r) => !completedSet.has(r)).length, updatedTs: l.updated_at ? new Date(l.updated_at).getTime() : 0 };
      }).sort((a, b) => a.unmet !== b.unmet ? a.unmet - b.unmet : b.updatedTs - a.updatedTs);
...
export async function isLessonUnlocked(lessonId: string, userId?: string) {
  ...
  return s ? !s.lockedLessons.find((l) => l.id === lessonId) : false;
}

export async function getNextAvailableLessons(userId?: string) {
  const s = await getCurriculumState(userId);
  if (!s) return null;
  return s.allLessons.filter((l) => (l.progress ?? 0) < 100 && !s.lockedLessons.find((x) => x.id === l.id)).sort((a, b) => (a.updated_at ? new Date(a.updated_at).getTime() : 0) - (b.updated_at ? new Date(b.updated_at).getTime() : 0));
}
