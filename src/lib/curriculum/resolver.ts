...
export function resolveCurriculumContext(input: { learner: LearnerCurriculumContext; versions: CurriculumVersionRecord[]; objectives: CurriculumObjective[]; mappings: ObjectiveSkillMapping[]; knowledge?: CurriculumKnowledgeItem[]; asOf?: string; }): ResolvedCurriculumContext {
  ...
  return { status: "resolved", reason: "Exact verified curriculum context and all required whole-syllabus knowledge layers resolved successfully.", curriculum: identity, versionId: version.id, objectives: resolvedObjectives, mappings: resolvedMappings, knowledge: resolvedKnowledge, knowledgeByKind: groupKnowledge(resolvedKnowledge) };
}
