...
import { EXAM_HISTORY_DIMENSIONS, requiredDimensionsForTier, type CurriculumCompletenessDimension, type CurriculumGateTier } from "./completeness";
...
export interface ResolvedCurriculumContext { status: "resolved" | "unverified"; reason: string; curriculum?: CurriculumIdentity; versionId?: string; objectives: CurriculumObjective[]; mappings: ObjectiveSkillMapping[]; knowledge: CurriculumKnowledgeItem[]; knowledgeByKind: Partial<Record<CurriculumKnowledgeKind, CurriculumKnowledgeItem[]>>;
  /** Which gate this resolution passed (default "exam"). */
  tier?: CurriculumGateTier;
  /** True only when all four exam-history dimensions are verified/not applicable. Consumers must not make exam-history claims unless true. */
  examHistoryVerified?: boolean;
  examHistoryMissing?: CurriculumCompletenessDimension[]; }
...
function missingCoverageDimensions(checks: CurriculumCoverageRecord[], dimensions: readonly CurriculumCompletenessDimension[]): CurriculumCompletenessDimension[] {
...
export function resolveCurriculumContext(input: { learner: LearnerCurriculumContext; versions: CurriculumVersionRecord[]; objectives: CurriculumObjective[]; mappings: ObjectiveSkillMapping[]; knowledge?: CurriculumKnowledgeItem[]; coverageChecks?: CurriculumCoverageRecord[]; asOf?: string; tier?: CurriculumGateTier; }): ResolvedCurriculumContext {
  const { learner, versions, objectives, mappings } = input;
  const asOf = input.asOf ?? new Date().toISOString().slice(0, 10);
  const tier: CurriculumGateTier = input.tier ?? "exam";
  ...
  const missingCoverage = missingCoverageDimensions(coverageChecks, requiredDimensionsForTier(tier));
  ...
  const examHistoryMissing = missingCoverageDimensions(coverageChecks, EXAM_HISTORY_DIMENSIONS);
  const examHistoryVerified = examHistoryMissing.length === 0;
  const reason = examHistoryVerified
    ? "Exact verified curriculum identity, coverage evidence and whole-syllabus knowledge layers resolved successfully."
    : `Exact verified curriculum identity, syllabus coverage and whole-syllabus knowledge layers resolved successfully. Exam-history claims (past papers, mark schemes, examiner reports, grade thresholds) remain blocked: not verified (${examHistoryMissing.join(", ")}).`;
  return { status: "resolved", reason, curriculum: identity, versionId: version.id, objectives: resolvedObjectives, mappings: resolvedMappings, knowledge: resolvedKnowledge, knowledgeByKind: groupKnowledge(resolvedKnowledge), tier, examHistoryVerified, examHistoryMissing };
}
