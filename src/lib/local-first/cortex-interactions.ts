...
export async function listLocalCortexInteractions(userId: string, limit = MAX_PER_USER): Promise<MemoryEntry[]> { requireUser(userId); const records = await localFirstStore.list<MemoryEntry>("insight", userId); return records.filter((record) => record.id.startsWith(`cortex-interaction:${userId}:`) && !record.deletedAt && record.payload?.userId === userId).sort((a, b) => new Date(b.payload.timestamp).getTime() - new Date(a.payload.timestamp).getTime()).slice(0, Math.max(1, Math.min(limit, MAX_PER_USER))).map((record) => record.payload); }
