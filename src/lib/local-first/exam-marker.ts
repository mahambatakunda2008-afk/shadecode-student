...
function findAnswer(answers: ExamAnswer[], id: number) {
...
export function markExamOffline(questions: MarkableQuestion[], answers: ExamAnswer[], timeTaken: number): LocalExamResults {
  let totalScore = 0;
  let maxScore = 0;
  ...
  for (const question of questions) {
    ...
    if (question.type === "multiple_choice" && question.modelAnswer) {
      ...
      totalScore += score;
      results.push({ questionId: question.id, score, maxScore: maxScoreForQuestion, correct, feedback: correct ? "Correct." : "Incorrect. Review this question when connected for detailed feedback.", modelAnswer: question.modelAnswer, topic: question.topic });
      continue;
    }

    pendingTopics.add(question.topic || "General");
    results.push({ questionId: question.id, score: 0, maxScore: maxScoreForQuestion, correct: false, feedback: "Saved for marking. This answer needs examiner/Cortex evaluation for reliable partial credit.", modelAnswer: question.modelAnswer ?? "", topic: question.topic });
  }
  ...
  const ranked = [...topicScores.entries()].map(([topic, value]) => ({ topic, percentage: value.max ? Math.round((value.score / value.max) * 100) : 0 })).sort((a, b) => b.percentage - a.percentage);
  ...
  return {
    ...
    cortexInsight: pendingTopics.size ? "Your objective answers were marked locally. Written answers remain queued for deeper examiner/Cortex marking when available." : "Your objective answers were marked locally and are ready for review.",
    results,
...
export const markExamLocally = markExamOffline;
