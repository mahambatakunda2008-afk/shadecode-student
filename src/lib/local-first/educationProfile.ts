...
export async function clearLocalEducationProfile(userId: string): Promise<void> {
  if (!userId) return;
  const record = await localFirstDB.getRecord(idFor(userId));
