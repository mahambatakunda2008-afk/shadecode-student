...
export async function saveLocalCortexMemory(
  userId: string,
  memory: CortexUserMemory,
): Promise<LocalRecord<CortexUserMemory>> {
  requireUser(userId);
  const normalized: CortexUserMemory = {
    ...memory,
    level: Math.max(1, Number.isFinite(memory.level) ? memory.level : 1),
    streak: Math.max(0, Number.isFinite(memory.streak) ? memory.streak : 0),
    xp: Math.max(0, Number.isFinite(memory.xp) ? memory.xp : 0),
    subjects: Array.isArray(memory.subjects) ? [...memory.subjects] : [],
    weakTopics: Array.isArray(memory.weakTopics) ? [...memory.weakTopics] : [],
    weakSubjects: Array.isArray(memory.weakSubjects) ? [...memory.weakSubjects] : [],
    frequentlyStudiedSubjects: Array.isArray(memory.frequentlyStudiedSubjects) ? [...memory.frequentlyStudiedSubjects] : [],
    strongSubjects: Array.isArray(memory.strongSubjects) ? [...memory.strongSubjects] : [],
    preferredStudyHours: Array.isArray(memory.preferredStudyHours) ? [...memory.preferredStudyHours] : [],
    examScores: Array.isArray(memory.examScores) ? [...memory.examScores] : [],
  };
  return localFirstStore.upsert({
    id: recordId(userId),
    entity: "insight",
    entityId: recordId(userId),
    userId,
    payload: normalized,
  });
}
...
export async function hydrateLocalCortexMemory(
  userId: string,
  memory: CortexUserMemory,
  updatedAt = Date.now(),
): Promise<void> {
  requireUser(userId);
  await localFirstStore.hydrate({
    id: recordId(userId),
    entity: "insight",
    entityId: recordId(userId),
    userId,
