...
class LocalFirstDB {
  ...
  private async transaction<T>(storeName: string, mode: IDBTransactionMode, run: (store: IDBObjectStore, resolve: (value: T) => void, reject: (reason?: unknown) => void) => void): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      run(transaction.objectStore(storeName), resolve, reject);
  ...
  async getOrCreateDeviceId(createId: () => string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (typeof value === "string" && value) { deviceId = value; return; }
        deviceId = createId();
  ...
  async mutateRecord<T>(input: { id: string; entity: LocalRecord["entity"]; entityId?: string; userId: string; payload: T; deviceId: string; deletedAt?: number }): Promise<LocalRecord<T>> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const records = transaction.objectStore(STORES.records), operations = transaction.objectStore(STORES.operations), meta = transaction.objectStore(STORES.meta);
      const existingRequest = records.get(input.id), sequenceKey = `${SEQUENCE_PREFIX}${input.deviceId}`;
      let existing: LocalRecord | undefined, sequence = 0, lamport = 0, result: LocalRecord<T> | undefined, ready = 0, settled = false;
      ...
      existingRequest.onsuccess = () => {
        ...
        const lamportRequest = meta.get(LAMPORT_KEY), sequenceRequest = meta.get(sequenceKey);
        const finish = () => {
      ...
      transaction.oncomplete = () => { if (!settled) { settled = true; result ? resolve(result) : reject(new Error("Local mutation completed without a result")); } };
  ...
  async getRecord<T = unknown>(id: string): Promise<LocalRecord<T> | null> { return this.transaction(STORES.records, "readonly", (store, resolve, reject) => { const request = store.get(id); request.onsuccess = () => resolve(request.result ?? null); request.onerror = () => reject(request.error); }); }
  async getRecords(userId: string): Promise<LocalRecord[]> { return this.transaction(STORES.records, "readonly", (store, resolve, reject) => { const request = store.index("userId").getAll(userId); request.onsuccess = () => resolve(request.result ?? []); request.onerror = () => reject(request.error); }); }
  ...
  async getOperations(userId: string): Promise<LocalOperation[]> { return (await this.transaction<LocalOperation[]>(STORES.operations, "readonly", (store, resolve, reject) => { const request = store.index("userId").getAll(userId); request.onsuccess = () => resolve(request.result ?? []); request.onerror = () => reject(request.error); })).filter((operation) => operation?.kind); }
  ...
  async getMeta(key: string): Promise<LocalMeta | null> { return this.transaction(STORES.meta, "readonly", (store, resolve, reject) => { const request = store.get(key); request.onsuccess = () => resolve(request.result ?? null); request.onerror = () => reject(request.error); }); }
  async clearUser(userId: string): Promise<void> { const records = await this.getRecords(userId), operations = await this.getOperations(userId); await this.init(); await new Promise<void>((resolve, reject) => { const transaction = this.db!.transaction([STORES.records, STORES.operations], "readwrite"), recordStore = transaction.objectStore(STORES.records), operationStore = transaction.objectStore(STORES.operations); records.forEach((record) => recordStore.delete(record.id)); operations.forEach((operation) => operationStore.delete(operation.id)); transaction.oncomplete = () => resolve(); transaction.onerror = () => reject(transaction.error); }); }
}
