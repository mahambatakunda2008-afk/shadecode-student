...
function readCachedProfile(userId: string): UserProfile | null {
  ...
  try { const raw = window.localStorage.getItem(`${PROFILE_CACHE_PREFIX}${userId}`); return raw ? JSON.parse(raw) as UserProfile : null; } catch { return null; }
}
function cacheProfile(profile: UserProfile): void {
  if (typeof window === "undefined") return;
  try { window.localStorage.setItem(`${PROFILE_CACHE_PREFIX}${profile.id}`, JSON.stringify(profile)); } catch {}
}
async function withTimeout<T>(promise: PromiseLike<T>, timeoutMs: number): Promise<T> {
  let timer: ReturnType<typeof setTimeout> | undefined;
  try {
    return await Promise.race([promise, new Promise<T>((_, reject) => { timer = setTimeout(() => reject(new Error("Timed out")), timeoutMs); })]);
...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const fetchProfile = useCallback(async (userId: string) => {
    ...
    try {
      ...
      cacheProfile(nextProfile); setProfile(nextProfile);
    } catch (error) { console.warn("[UserContext] profile hydration skipped:", error); }
  }, [supabase]);
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (!mounted) return;
      if (event === "SIGNED_OUT" || !session) { offlineSync.stopAutoSync(); setUser(null); setProfile(null); setLoading(false); return; }
      if (event === "SIGNED_IN" || event === "TOKEN_REFRESHED") { setUser(session.user); setLoading(false); void fetchProfile(session.user.id); }
    });
