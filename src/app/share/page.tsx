...
import { ArrowRight, Check, Copy, MessageCircle, Share2, Sparkles } from "lucide-react";
...
export default function SharePage() {
  ...
  useEffect(() => {
    ...
    if (referral) {
      const safeReferral = referral.slice(0, 80);
      try { sessionStorage.setItem("shadecode_referral", safeReferral); } catch {}
      void trackEvent("referral_landed", { referral: safeReferral });
    }
    ...
    void client.auth.getUser().then(async ({ data }) => {
      ...
      const [{ data: profile }, { data: activity }] = await Promise.all([
        client.from("student_progress").select("overall_completion").eq("user_id", data.user.id).maybeSingle(),
        client.from("student_activity").select("current_streak").eq("user_id", data.user.id).maybeSingle(),
      ]);
      ...
      void trackEvent("share_hub_opened", { hasReferral: Boolean(referral) });
    });
  }, []);

  const shareBase = typeof window === "undefined" ? "https://shadecodestudent.vercel.app" : window.location.origin;
  const headline = progress !== null ? `${name} has reached ${progress}% overall study progress on Shadecode Student.` : authenticated ? `${name} is building a study streak with Shadecode Student.` : "Study, practise and track your progress with Shadecode Student.";
  const streakLine = streak !== null && streak > 0 ? ` Current streak: ${streak} day${streak === 1 ? "" : "s"}.` : "";

  const copyQuick = async () => {
    const value = `${shareBase}/share`;
    try {
