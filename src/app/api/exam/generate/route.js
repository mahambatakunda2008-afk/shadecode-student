...
async function getLearnerContext(req) {
  const cookie = req.headers.get("cookie");
  const contextUrl = new URL("/api/learner/context", req.url);
...
export async function POST(req) {
  try {
    ...
    const learner = await getLearnerContext(req);
    ...
    const exam = await generateExam(subject, topics, difficulty, questionCount, userId, curriculum);
    if (!exam) return NextResponse.json({ error: "Couldn't generate a sufficiently reliable exam right now. Please try again in a moment." }, { status: 503 });

    createSupabaseServerClient().then(supabase => {
      supabase.from('exams').insert({ user_id: userId, subject, difficulty, questions: exam.questions })
        .then(({ error }) => { if (error) console.error("[exam/generate] Background save failed:", error.message); });
    });

    return NextResponse.json({ questions: exam.questions, metadata: { subject, topic: topic ?? subject, stage: curriculum.stage, board: curriculum.board, qualification: curriculum.qualification, syllabusCode: curriculum.syllabusCode, syllabusYear: curriculum.syllabusYear } });
  } catch (err) {
