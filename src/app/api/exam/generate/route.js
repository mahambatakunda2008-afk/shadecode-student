...
export async function POST(req) {
  try {
    ...
    if (!learner) {
      return NextResponse.json({ error: "Your academic profile is incomplete. Finish onboarding before generating an exam." }, { status: 409 });
    }

    const curriculum = resolveCurriculum(learner, subject, topic);
    if (!curriculum) {
      return NextResponse.json({ error: "That subject is outside your enrolled academic scope." }, { status: 403 });
    }
