...
function lessonPrompt(request: ReturnType<typeof resolveLessonRequest>, curriculumContext = "") {
  ...
  const sequences: Record<typeof request.intent, string> = { teach: "objective → prerequisite → core concept → explanation → formula/method → worked example → checkpoint → misconception → exam transfer → practice → summary → next step", remedial: "objective → diagnose confusion → prerequisite → plain explanation → worked example → misconception/correction → checkpoint → second example → exam transfer → guided practice → summary → next step", revision: "objective → prerequisite recap → key ideas → definitions/formulas → worked example → high-yield patterns → checkpoint → common mistakes → exam transfer → practice → summary → next step", practice: "objective → method → worked example → question 1 → question 2 → question 3 → feedback guidance → exam transfer → traps → summary → next step", comparison: "objective → define both → explicit comparison → similarities → differences → worked application → misconception → checkpoint → exam transfer → practice → summary → next step" };
  return `You are the senior teaching engine inside Shadecode Student. Produce a premium, rigorous learning session, not an AI-generated article. The learner should finish able to explain the idea, use it, detect mistakes, and perform under assessment conditions.
...
- Explicit education level and exam board are authoritative. If verified curriculum context is present, its objectives define scope. Never invent syllabus requirements.
- Teach the mental model first. A learner must understand WHY before being asked to memorize WHAT.
- Introduce prerequisite knowledge only when it unlocks the target topic. Keep prerequisites brief and relevant.
- Define every technical term at first use. Then use it naturally rather than repeating the definition.
- Examples must contain reasoning, not just a final answer. For calculations show substitution, units, intermediate steps and the final interpretation.
...
STRUCTURE
Return 10-16 blocks, with this minimum backbone: objective, prerequisite or concept, definition/concept explanation, formula/method when relevant, worked example, checkpoint, misconception/mistake, exam application, practice, summary. Comparison adds comparison. Remedial adds misconception. Revision adds high-yield exam patterns. Practice adds at least 3 questions.

RETURN ONLY JSON
{"title":"specific outcome-focused title","blocks":[{"type":"objective|prior|concept|definition|formula|example|checkpoint|comparison|misconception|exam|mistake|summary|practice|tip","title":"short heading","content":"substantive student-facing content"}]}
...
async function generateAndValidate(request: ReturnType<typeof resolveLessonRequest>, curriculumContext: string, userId: string) {
  ...
  const repair = await callAI(`You are repairing a failed premium lesson draft. Return ONLY valid JSON. Preserve the exact topic, subject, level, board and intent. Do not merely paraphrase the draft. Rebuild weak sections with concrete teaching.

REQUIRED: 10-16 distinct blocks; objective; concept; definition where useful; formula/method when relevant; worked example with reasoning; checkpoint WITHOUT its answer; misconception/trap; exam transfer; progressive practice; summary. Practice intent requires at least 3 progressively harder questions. Comparison requires a real comparison block. Quantitative subjects normally require a formula block. Remove repetition, generic filler, fake claims, and motivational fluff.
