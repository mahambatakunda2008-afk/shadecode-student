...
function buildLessonPrompt(subject: string, topic: string, difficulty: string): string {
  const guide = ({ easy: "Use guided first principles, small steps and frequent checks.", medium: "Use rigorous reasoning, precise terminology and exam application appropriate to the learner's curriculum level.", hard: "Use demanding exam-ready reasoning, subtle distinctions, traps and higher-order application." } as Record<string,string>)[difficulty] ?? "Use rigorous reasoning, precise terminology and exam application appropriate to the learner's curriculum level.";
...
The learner's verified academic curriculum context, when available, will be appended by the server. Treat that context as authoritative for board, level, syllabus scope and objectives. Never invent syllabus requirements. If verified curriculum context is absent, teach the topic accurately as general educational material without claiming board alignment.
...
10. exam: give an application or structured question appropriate to the learner's verified board/level when provided, then explain the method and mark-worthy reasoning.
...
export async function POST(req: Request) { let auth: any = null; try { const rateLimitCheck = await applyRateLimit(req, aiEndpointLimiter); if (rateLimitCheck) return rateLimitCheck; auth = await authenticateRequest(req); if (!auth) return NextResponse.json({ error: "Unauthorized" }, { status: 401 }); const { supabase, user } = auth; const body = await req.json(); const { type, subject, topic, difficulty, goal, level } = body; if (type === "course_preview") { const validation = validateRequestBody({ topic, goal, level }, learnCoursePreviewSchema); if (!validation.success) return NextResponse.json({ error: "Validation failed", details: validation.details?.issues.map((e: any) => ({ field: e.path.join('.'), message: e.message })) }, { status: 400 }); } else if (type === "generate_lesson") { const validation = validateRequestBody({ type, subject, topic, difficulty }, learnGenerateLessonSchema); if (!validation.success) return NextResponse.json({ error: "Validation failed", details: validation.details?.issues.map((e: any) => ({ field: e.path.join('.'), message: e.message })) }, { status: 400 }); }
    ...
    // The UI/schema uses `generate_lesson`; older callers may still use `lesson`.
    ...
    let raw = await callAI(prompt, 7000, { userId: user.id, feature: "lesson_assistant", subfeature: "generate_lesson" });
    ...
    if (!parsed) {
      ...
      const repaired = await callAI(repairPrompt, 5000, { userId: user.id, feature: "lesson_assistant", subfeature: "repair_lesson_json" });
      if (repaired) { parsed = safeParseJSON(repaired); raw = repaired; }
    }
    if (!parsed?.blocks.length) { log.lessonGenerationFailed({ userId: user.id, subject, topic, difficulty: validDifficulty, error: `AI returned invalid lesson JSON: ${raw.slice(0, 300)}` }); return NextResponse.json({ error: "Couldn't generate a complete lesson on that topic. Please try again or rephrase it." }, { status: 422 }); }
    let subjectId: string | null = null;
    const { data: existingSubject } = await supabase.from("subjects").select("id").eq("user_id", user.id).eq("name", subject).maybeSingle();
    if (existingSubject?.id) subjectId = existingSubject.id;
    else { const { data: newSubject } = await supabase.from("subjects").insert({ user_id: user.id, name: subject }).select("id").single(); subjectId = newSubject?.id ?? null; }
    let savedId: string | null = null;
    if (subjectId) {
      const { data: inserted, error: insertError } = await supabase.from("learn_lessons").insert({ user_id: user.id, subject_id: subjectId, topic: topic.trim().slice(0,500), title: parsed.title, description: `A complete ${validDifficulty} lesson on ${topic.trim()}`, difficulty: validDifficulty, progress: 0, blocks: parsed.blocks }).select("id").single();
      if (!insertError) { savedId = inserted.id; await awardXPBySource(user.id, "lesson_generation", { difficulty: validDifficulty }); }
      else log.lessonGenerationFailed({ userId: user.id, subject, topic, difficulty: validDifficulty, error: `Failed to save lesson: ${insertError.message}` });
    }
    if (!savedId) return NextResponse.json({ error: "The lesson was generated but couldn't be saved. Please try again." }, { status: 500 });
    return NextResponse.json({ id: savedId, title: parsed.title, blocks: parsed.blocks });
  } catch (err: any) { log.apiFailure({ route: "/api/learn", method: "POST", error: err.message || String(err), userId: auth?.user?.id }); return NextResponse.json({ error: "Something went wrong" }, { status: 500 }); } }
