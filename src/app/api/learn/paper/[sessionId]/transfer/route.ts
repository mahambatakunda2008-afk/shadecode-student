...
export async function POST(req: Request, context: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    if (transfer.submit_action_id && transfer.submit_action_id !== clientActionId) {
      return NextResponse.json({ error: "This transfer is already being graded. Retry the same submission shortly." }, { status: 409 });
    }
    if (!transfer.submit_action_id) {
      const { data: claimedForGrading } = await auth.supabase.from("paper_learning_transfer_questions")
        .update({ submit_action_id: clientActionId, grading_started_at: new Date().toISOString() })
        .eq("id", transferId).eq("session_id", sessionId).eq("user_id", auth.user.id)
        .eq("status", "pending").is("submit_action_id", null)
        .select("id,submit_action_id").maybeSingle();
      if (!claimedForGrading) {
        const { data: race } = await auth.supabase.from("paper_learning_transfer_questions")
          .select("status,submit_action_id,verdict,feedback,misconception,next_action")
          .eq("id", transferId).eq("session_id", sessionId).eq("user_id", auth.user.id).maybeSingle();
