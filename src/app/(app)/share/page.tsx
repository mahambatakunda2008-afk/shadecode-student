...
export default function SharePage() {
  const router = useRouter(); const searchParams = useSearchParams(); const [name, setName] = useState("Student"); const [progress, setProgress] = useState<number | null>(null); const [streak, setStreak] = useState<number | null>(null);
  useEffect(() => {
    const referral = searchParams.get("ref"); if (referral) { try { sessionStorage.setItem("shadecode_referral", referral.slice(0, 80)); } catch {} void trackEvent("referral_landed", { referral }); }
    const client = createClient();
    void client.auth.getUser().then(async ({ data }) => {
      if (!data.user) { router.replace("/auth/login?next=/share"); return; }
  ...
  const headline = progress !== null ? `${name} has reached ${progress}% overall study progress on Shadecode Student.` : `${name} is building a study streak with Shadecode Student.`; const streakLine = streak !== null && streak > 0 ? ` Current streak: ${streak} day${streak === 1 ? "" : "s"}.` : "";
  const shareBase = typeof window === "undefined" ? "https://shadecodestudent.vercel.app" : window.location.origin;
  const copyQuick = async () => { try { await navigator.clipboard?.writeText(`${shareBase}/share`); void trackEvent("share_completed", { shareType: "quick", method: "copy" }); } catch {} };
  return <main className="mx-auto w-full max-w-3xl px-4 py-6 sm:px-6"><button type="button" onClick={() => router.back()} className="mb-6 inline-flex items-center gap-2 text-sm text-[var(--muted-foreground)]"><ArrowLeft size={16} /> Back</button><div className="mb-6 rounded-3xl border border-[var(--card-border)] bg-[var(--surface-2)] p-6 sm:p-8"><div className="mb-3 flex items-center gap-2 text-sm font-semibold text-[var(--primary)]"><Sparkles size={16} /> Growth hub</div><h1 className="text-3xl font-black tracking-tight">Turn progress into momentum.</h1><p className="mt-2 max-w-xl text-sm leading-6 text-[var(--muted-foreground)]">Share something real from your study journey. The goal is simple: help another student discover a useful study tool.</p></div><div className="grid gap-4"><ShareCard shareType="progress" title="Shadecode Student progress" text={`${headline}${streakLine}`} /><ShareCard shareType="study_invite" title="Study with Shadecode Student" text="I'm using Shadecode Student to organise learning, practise and track progress. Try it with me." /></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><QuickAction icon={<Share2 size={17} />} label="Share" onClick={() => navigator.share ? void navigator.share({ title: "Shadecode Student", text: "Study smarter with Shadecode Student.", url: `${shareBase}/share` }) : undefined} /><QuickAction icon={<MessageCircle size={17} />} label="WhatsApp" onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent("Study smarter with Shadecode Student: " + shareBase + "/share")}`, "_blank", "noopener,noreferrer")} /><QuickAction icon={<Copy size={17} />} label="Copy link" onClick={() => void copyQuick()} /></div></main>;
}
