...
function recommendationScore(activity: Activity, mastery: Mastery | undefined) {
  ...
  return ((1 - masteryScore) * 0.5 + errorRate * 0.2 + uncertainty * 0.2 + (1 - confidence) * 0.1 + (1 - improvement) * 0.05) * 100 + activity.sort_order / 10000;
}
...
export default function DiscoveryHome() {
  ...
  useEffect(() => {
    ...
    async function syncQueuedProgress() {
      ...
      try {
        ...
        for (const item of ownQueue) {
          const { error } = await supabase.from("primary_activity_progress").upsert({ user_id: learnerId, activity_id: item.activityId, progress: item.progress, completed: item.completed, attempt_count: item.attempt_count, last_completed_at: item.last_completed_at, updated_at: new Date().toISOString() }, { onConflict: "user_id,activity_id" });
    ...
    async function load() {
      ...
      if (cached) { setActivities(cached.activities); setProgress(cached.progress ?? []); setMastery(cached.mastery ?? []); }
      ...
      if (!usable.length && cached?.activities?.length) { setOffline(true); setLoading(false); return; }
      ...
      if (usable.length) {
        ...
        if (!cancelled) { const nextProgress = (rows ?? []) as Progress[]; const nextMastery = (masteryRows ?? []) as Mastery[]; setProgress(nextProgress); setMastery(nextMastery); writeHomeCache(learnerId, { activities: usable, progress: nextProgress, mastery: nextMastery }); }
      } else writeHomeCache(learnerId, { activities: usable, progress: [], mastery: [] });
  ...
  const next = useMemo(() => { const unfinished = activities.filter((item) => !progressMap.get(item.id)?.completed); if (!unfinished.length) return activities[0]; const hasMastery = unfinished.some((item) => masteryMap.has(`${item.subject.toLowerCase()}::${item.topic.toLowerCase()}`)); if (!hasMastery) return unfinished[0]; return unfinished.reduce((best, item) => { const bestMastery = masteryMap.get(`${best.subject.toLowerCase()}::${best.topic.toLowerCase()}`); const itemMastery = masteryMap.get(`${item.subject.toLowerCase()}::${item.topic.toLowerCase()}`); return recommendationScore(item, itemMastery) > recommendationScore(best, bestMastery) ? item : best; }); }, [activities, masteryMap, progressMap]);
  ...
  const recommendationLabel = nextMastery ? Number(nextMastery.mastery_score ?? 0) < 60 ? "A skill to strengthen" : Number(nextMastery.uncertainty) > 0.5 ? "A skill worth exploring" : "Your next step" : "Your next step";

  return <main className="mx-auto max-w-5xl px-4 py-6 md:px-6 md:py-10">
    <header className="relative overflow-hidden rounded-[30px] border border-[var(--card-border)] bg-[var(--surface)] p-7 shadow-sm md:p-10"><div className="pointer-events-none absolute -right-24 -top-24 h-64 w-64 rounded-full bg-[var(--primary-glow)] blur-3xl" /><div className="relative flex flex-col gap-6 md:flex-row md:items-end md:justify-between"><div className="max-w-2xl"><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-[var(--primary)]"><Compass className="h-4 w-4" /> Shadecode Discovery</p><h1 className="mt-2 text-3xl font-black tracking-tight text-[var(--foreground)] md:text-4xl">My Day starts here. 🌱</h1><p className="mt-3 text-sm leading-6 text-[var(--muted-foreground)]">Short adventures, real progress, and a next step chosen from your learning evidence.</p></div><div className="flex items-center gap-3 text-xs font-semibold text-[var(--muted-foreground)]"><span>{completed}/{activities.length} complete</span>{offline && <span className="inline-flex items-center gap-1.5"><CloudOff className="h-4 w-4" /> Offline</span>}</div></div></header>
    {next && <section className="mt-6 rounded-[26px] border border-[var(--card-border)] bg-[var(--surface)] p-6 shadow-sm md:p-7"><div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--primary)]">{recommendationLabel}</p><h2 className="mt-1 text-2xl font-bold text-[var(--foreground)]">{next.title}</h2><p className="mt-2 text-sm text-[var(--muted-foreground)]">{next.subject} · {next.skill}</p></div><Link href={`/discovery/activity/${next.id}`} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--primary)] px-5 py-3 text-sm font-bold text-white">Start adventure <ArrowRight className="h-4 w-4" /></Link></div></section>}
