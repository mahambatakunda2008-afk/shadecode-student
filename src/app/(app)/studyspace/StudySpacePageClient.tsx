  ...
  return (
    <main style={{ padding: "24px", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(220px, 280px) 1fr", gap: 18 }}>
        <nav aria-label="StudySpace modes" style={{ display: "grid", gap: 8, alignContent: "start" }}>{modes.map((item) => <button key={item.id} onClick={() => setMode(item.id)} aria-current={mode === item.id ? "page" : undefined} style={{ textAlign: "left", border: "1px solid var(--card-border)", borderRadius: 10, padding: 13, background: mode === item.id ? "var(--primary)" : "var(--card)", color: "var(--foreground)", cursor: "pointer" }}><strong>{item.label}</strong><span style={{ display: "block", fontSize: 12, marginTop: 4, opacity: 0.8 }}>{item.description}</span></button>)}</nav>
        <section style={{ border: "1px solid var(--card-border)", borderRadius: 14, padding: 20, background: "var(--card)" }} aria-label="StudySpace work surface">
          <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}><div><h2 style={{ margin: 0 }}>{current.label}</h2><p style={{ margin: "5px 0 18px", color: "var(--muted-foreground)" }}>{current.description}</p></div><span style={{ fontSize: 12, color: "var(--muted-foreground)" }}>Saved locally</span></div>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: showCanvas || showCalculator ? 18 : 0 }}><button onClick={save} style={{ border: 0, borderRadius: 8, padding: "11px 16px", background: "var(--primary)", color: "white", fontWeight: 750, cursor: "pointer" }}>Save work</button>{mode !== "canvas" && <button type="button" onClick={() => setShowCanvas((value) => !value)} style={{ border: "1px solid var(--card-border)", borderRadius: 8, padding: "10px 14px", background: "var(--muted)", color: "var(--foreground)", cursor: "pointer" }}>{showCanvas ? "Hide Canvas" : "Open Canvas"}</button>}<button type="button" onClick={() => setShowCalculator((value) => !value)} aria-expanded={showCalculator} style={{ border: "1px solid var(--card-border)", borderRadius: 8, padding: "10px 14px", background: "var(--muted)", color: "var(--foreground)", cursor: "pointer" }}>{showCalculator ? "Hide Calculator" : "Open Calculator"}</button><button onClick={() => router.push(`/workmate?mode=${mode}`)} style={{ border: "1px solid var(--card-border)", borderRadius: 8, padding: "10px 14px", background: "var(--muted)", color: "var(--foreground)", cursor: "pointer" }}>Open Workmate</button>{saved && <span role="status" style={{ fontSize: 13, color: "var(--muted-foreground)" }}>Saved offline ✓</span>}</div>
