...
export default function StudyPlanPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Prefill from the profile the student already has, rather than
      // asking them to re-type subjects they've already told the app
      ...
      setPrefillSubjects((profile?.subjects as string[] | null) ?? []);
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "32px 20px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        ...
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 800, margin: 0 }}>Study Plan</h1>
          <p style={{ fontSize: 12, color: "var(--muted-foreground)", margin: 0 }}>
            A weighted weekly schedule built around your exam date and real weak topics
          </p>
        </div>
      </div>
      ...
      {loadError && !loading && (
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: 12, color: "#ef444499", margin: "2px 0 0" }}>{loadError}</p>
          </div>
          <button
            onClick={load}
            style={{
      ...
      {!loading && !loadError && !plan && (
        <>
          {submitError && (
            <p style={{ fontSize: 12, color: "#ef4444", margin: "0 0 12px" }}>{submitError}</p>
          )}
          <div style={{ opacity: submitting ? 0.6 : 1, pointerEvents: submitting ? "none" : "auto" }}>
            <StudyGoalInput
              onSubmit={handleGenerate}
              initialGoals={{ subjects: prefillSubjects }}
            />
          </div>
          {submitting && (
