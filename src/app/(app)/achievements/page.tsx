...
import {
  Trophy, Lock, Sparkles, Star, Flame, Medal, Crown, Loader2, Target,
...
const RARITY_ICONS: Record<string, React.ReactNode> = {
  common: <Medal className="w-4 h-4" />, rare: <Star className="w-4 h-4" />, epic: <Flame className="w-4 h-4" />, legendary: <Crown className="w-4 h-4" />,
};

const ACHIEVEMENT_ICONS: Record<string, LucideIcon> = {
  Target, ListChecks, Trophy, FileCheck2, GraduationCap, Flame, Sparkles, Search, Languages, BadgeCheck, Timer, BookOpen, Star, CalendarCheck,
  "🎯": Target, "📋": ListChecks, "🏆": Trophy, "📝": FileCheck2, "🎓": GraduationCap, "🔥": Flame, "💫": Sparkles, "🔍": Search, "🌐": Languages, "💯": BadgeCheck, "⏱️": Timer, "📚": BookOpen, "📖": BookOpen, "⭐": Star, "📅": CalendarCheck,
};
...
export default function AchievementsPage() {
  ...
  const copy = experience.family === "foundation"
    ? { title: "Milestones", subtitle: "See the things you have discovered, practised and completed.", progress: "Your growth", unlocked: "milestones reached", empty: "No milestones match this filter" }
    : experience.family === "beyond-school"
      ? { title: "Achievements", subtitle: "Track progress across your learning, work and practical goals.", progress: "Your progress", unlocked: "achievements unlocked", empty: "No achievements match this filter" }
      : { title: "Achievements", subtitle: "Track your study progress and unlock rewards as you learn.", progress: "Progress", unlocked: "unlocked", empty: "No achievements match this filter" };
      ...
      <div className="bg-[var(--card)] rounded-xl p-4 border border-[var(--card-border)]">
        ...
        <div className="flex gap-2 mt-3">
          {["common", "rare", "epic", "legendary"].map((rarity) => {
            ...
            return <div key={rarity} className={`flex-1 ${c.bg} rounded-lg p-2 border ${c.border} text-center`}><div className="flex items-center justify-center gap-1"><span className={c.text}>{RARITY_ICONS[rarity]}</span><span className={`text-xs capitalize ${c.text}`}>{rarity}</span></div><div className={`text-sm font-bold ${c.text}`}>{count}/{total}</div></div>;
          })}
        </div>
      </div>
