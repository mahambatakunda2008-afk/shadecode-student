          ...
          {blocks.map((block, index) => {
            ...
            return <article key={block.id || `${block.type}-${index}`} className={`rounded-2xl border bg-[var(--card)] p-5 sm:p-6 ${selected ? "border-[var(--primary)] shadow-sm" : "border-[var(--card-border)]"}`}>
              <button type="button" onClick={() => selectCheckpoint(block.id)} className="flex w-full items-start gap-3 text-left">
                ...
                <div className="min-w-0 flex-1"><p className="text-[10px] font-black uppercase tracking-wider text-[var(--primary)]">{done ? "mastered" : block.type.replace(/-/g, " ")}</p>{block.title && <h2 className="mt-1 text-lg font-bold">{block.title}</h2>}{guidedMode && ["method","example","application","mistake","pattern"].includes(block.type) && !revealedBlocks.includes(block.id) ? <div className="mt-2 rounded-xl border border-dashed border-[var(--card-border)] bg-[var(--surface)] p-3 text-xs text-[var(--muted-foreground)]">Reasoning hidden in Guided mode. Open the checkpoint when ready.</div> : <div className="mt-2 whitespace-pre-line text-sm leading-7">{block.content}</div>}{block.sourcePages?.length ? <p className="mt-2 text-[10px] font-semibold text-[var(--muted-foreground)]">Source pages: {block.sourcePages.join(", ")}</p> : null}</div>
