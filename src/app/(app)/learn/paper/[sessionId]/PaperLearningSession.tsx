                        ...
                        {transferResult.nextAction ? (
                          <>
                            ...
                            <div className="mt-4 flex flex-wrap gap-2">
                              <button type="button" onClick={() => router.push(`/learn?subject=${encodeURIComponent(session.learning_plan.subject || "")}&topic=${encodeURIComponent(block.title || transferResult.nextAction || "")}`)} className="inline-flex items-center gap-2 rounded-xl bg-[var(--primary)] px-3 py-2 text-xs font-bold text-[var(--primary-foreground)]"><Target className="h-3.5 w-3.5" /> Target this in Learn</button>
                              <button type="button" onClick={() => router.push(`/exam-sim?subject=${encodeURIComponent(session.learning_plan.subject || "")}&topic=${encodeURIComponent(block.title || transferResult.nextAction || "")}&count=5`)} className="inline-flex items-center gap-2 rounded-xl border border-[var(--card-border)] bg-[var(--surface)] px-3 py-2 text-xs font-bold"><GraduationCap className="h-3.5 w-3.5" /> Test it in Exam Sim</button>
                            </div>
                          </>
                        ) : null}
                      </div>
