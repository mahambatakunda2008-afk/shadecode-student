...
import { ArrowLeft, BookOpen, Check, CheckCircle2, ChevronRight, FileText, Lightbulb, Loader2, RotateCcw, Send, WifiOff, Target, GraduationCap } from "lucide-react";
          ...
          {blocks.map((block, index) => {
            ...
            return <article key={block.id || `${block.type}-${index}`} className={`rounded-2xl border bg-[var(--card)] p-5 sm:p-6 ${selected ? "border-[var(--primary)] shadow-sm" : "border-[var(--card-border)]"}`}>
              ...
              {selected && block.interaction?.prompt && !done && <div className="mt-5 border-t border-[var(--card-border)] pt-5">
                ...
                {result && <div className="mt-3 rounded-2xl border border-[var(--card-border)] bg-[var(--surface)] p-4"><p className="text-sm font-black uppercase tracking-wide">{result.verdict.replace("_", " ")}</p><p className="mt-2 text-sm leading-6">{result.feedback}</p>{result.misconception && <p className="mt-2 text-sm leading-6"><span className="font-bold">What to fix:</span> {result.misconception}</p>}{result.nextAction && <p className="mt-2 text-sm leading-6"><span className="font-bold">Next:</span> {result.nextAction}</p>}{result.nextAction && <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => router.push(`/learn?subject=${encodeURIComponent(session.learning_plan.subject || "")}&topic=${encodeURIComponent(currentBlock.title || result.nextAction || "")}`)} className="inline-flex items-center gap-2 rounded-xl bg-[var(--primary)] px-3 py-2 text-xs font-bold text-[var(--primary-foreground)]"><Target className="h-3.5 w-3.5" /> Continue in Learn</button><button type="button" onClick={() => router.push(`/exam-sim?subject=${encodeURIComponent(session.learning_plan.subject || "")}&topic=${encodeURIComponent(currentBlock.title || result.nextAction || "")}&count=5`)} className="inline-flex items-center gap-2 rounded-xl border border-[var(--card-border)] bg-[var(--surface)] px-3 py-2 text-xs font-bold"><GraduationCap className="h-3.5 w-3.5" /> Practise in Exam Sim</button></div>}{result.solution && <div className="mt-3 border-t border-[var(--card-border)] pt-3 text-sm leading-6"><span className="font-bold">Reference solution:</span><p className="mt-1 whitespace-pre-line">{result.solution}</p></div>}
                ...
                {transferQuestion && (
                  <div className="mt-3 rounded-2xl border border-[var(--card-border)] bg-[var(--surface)] p-4">
                    <p className="text-xs font-black uppercase tracking-wider text-[var(--primary)]">Transfer challenge</p>
                    <p className="mt-2 text-sm leading-7 whitespace-pre-line">{transferQuestion}</p>
                    <textarea value={transferAnswer} onChange={e => setTransferAnswer(e.target.value)} disabled={checking || offline || !!transferResult} rows={5} placeholder="Solve the new problem. Show your reasoning." className="mt-4 w-full resize-y rounded-2xl border border-[var(--card-border)] bg-[var(--card)] p-4 text-sm outline-none focus:border-[var(--primary)]" />
                    <button type="button" onClick={submitTransfer} disabled={checking || offline || !transferAnswer.trim() || !!transferResult} className="mt-3 inline-flex items-center gap-2 rounded-xl bg-[var(--primary)] px-4 py-2.5 text-sm font-bold text-[var(--primary-foreground)] disabled:opacity-50">
                      {checking ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />} Submit transfer
                    </button>
                    {transferResult ? (
                      <div className="mt-4 border-t border-[var(--card-border)] pt-4">
                        <p className="text-sm font-black uppercase tracking-wide">{transferResult.verdict?.replace("_", " ")}</p>
                        <p className="mt-2 text-sm leading-6">{transferResult.feedback}</p>
                        {transferResult.misconception ? <p className="mt-2 text-sm leading-6"><span className="font-bold">What to fix:</span> {transferResult.misconception}</p> : null}
                        {transferResult.nextAction ? (
                          <>
                            <p className="mt-2 text-sm leading-6"><span className="font-bold">Next:</span> {transferResult.nextAction}</p>
                            <div className="mt-4 flex flex-wrap gap-2">
                              <button type="button" onClick={() => router.push(`/learn?subject=${encodeURIComponent(session.learning_plan.subject || "")}&topic=${encodeURIComponent(currentBlock.title || transferResult.nextAction || "")}`)} className="inline-flex items-center gap-2 rounded-xl bg-[var(--primary)] px-3 py-2 text-xs font-bold text-[var(--primary-foreground)]"><Target className="h-3.5 w-3.5" /> Target this in Learn</button>
                              <button type="button" onClick={() => router.push(`/exam-sim?subject=${encodeURIComponent(session.learning_plan.subject || "")}&topic=${encodeURIComponent(currentBlock.title || transferResult.nextAction || "")}&count=5`)} className="inline-flex items-center gap-2 rounded-xl border border-[var(--card-border)] bg-[var(--surface)] px-3 py-2 text-xs font-bold"><GraduationCap className="h-3.5 w-3.5" /> Test it in Exam Sim</button>
                            </div>
                          </>
                        ) : null}
                      </div>
