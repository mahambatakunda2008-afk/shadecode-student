...
const CACHE_KEY = "shadecode:learn:list:v2";
const REQUEST_TIMEOUT_MS = 12000;
const GENERATE_TIMEOUT_MS = 65000;

type Difficulty = "easy" | "medium" | "hard";
type Lesson = { id: string; subjectId: string; subject: string; title: string; description?: string; difficulty: Difficulty; progress: number; completed: boolean; updated_at?: string };
type Subject = { id: string; name: string; lessonCount: number };
type Summary = { currentXP: number; currentStreak: number; level: number; xpGoal: number };
type LearnData = { subjects: Subject[]; lessons: Lesson[]; summary: Summary | null };
...
function readCache(): LearnData | null {
  try { const value = JSON.parse(localStorage.getItem(CACHE_KEY) || "null"); return value?.subjects && value?.lessons ? value : null; } catch { return null; }
}

function writeCache(data: LearnData) {
  try { localStorage.setItem(CACHE_KEY, JSON.stringify(data)); } catch { /* storage is optional */ }
}
...
export default function LearnPageClientV2() {
  ...
  const load = useCallback(async (accessToken?: string, silent = false) => {
    const cached = readCache();
    if (cached && !accessToken) setData(cached);
    if (!silent) setLoading(!cached);
    setRefreshing(true);
    setError(null);
    if (!navigator.onLine) { setOffline(true); setLoading(false); setRefreshing(false); return; }
    try {
      const sessionToken = accessToken ?? token;
      if (!sessionToken) throw new Error("SESSION_MISSING");
      const fresh = await requestJson<LearnData>("/api/learn", { headers: { Authorization: `Bearer ${sessionToken}` } }, REQUEST_TIMEOUT_MS);
      setData(fresh); writeCache(fresh); setOffline(false);
    } catch (e) {
      const message = e instanceof Error ? e.message : "Unable to load Learn.";
      if (message === "SESSION_MISSING") { router.replace("/login"); return; }
      setOffline(!navigator.onLine || message === "REQUEST_TIMEOUT");
      if (!cached) setError(message === "REQUEST_TIMEOUT" ? "Learn took too long to respond. Your cached lessons are still available after retry." : "Couldn't load your lessons. Please retry.");
  ...
  useEffect(() => {
    ...
    const online = () => { setOffline(false); void getSessionWithTimeout().then((s) => s && load(s.access_token, true)).catch(() => undefined); };
    ...
    <main className="min-h-screen bg-[var(--background)] px-4 py-6 text-[var(--foreground)] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="rounded-3xl border border-[var(--card-border)] bg-[var(--card)] p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-3 flex flex-wrap gap-2" role="radiogroup" aria-label="Lesson difficulty">{([['easy','Guided'],['medium','Standard'],['hard','Challenge']] as const).map(([value, label]) => <button key={value} type="button" role="radio" aria-checked={difficulty === value} onClick={() => setDifficulty(value)} className={`rounded-lg border px-3 py-2 text-xs font-semibold ${difficulty === value ? "border-[var(--primary)] bg-[var(--primary-glow)] text-[var(--primary)]" : "border-[var(--card-border)] text-[var(--muted-foreground)]"}`}>{label}</button>)}</div>
