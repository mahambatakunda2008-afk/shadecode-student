    ...
    <main className="min-h-screen bg-[#0B0D12] px-5 py-10 text-white sm:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3" aria-label="Download options">
          <PlatformCard icon="🌐" title="Web" status="Available now" description="Works immediately in Chrome, Edge, Safari, Firefox, and other modern browsers. Nothing to install." action="Open Shadecode Student" href={WEB_URL} external />
          <PlatformCard icon="📱" title="Android APK" status="Release pipeline ready" description="The official Android build is produced by the Shadecode release pipeline. Direct APK distribution becomes available as soon as the first signed production release is published." action="Open official releases" href={ANDROID_RELEASES_URL} external secondary="Only install APKs published by Shadecode." />
          <PlatformCard icon="▶️" title="Google Play" status="Publishing next" description="The Android project produces the AAB required for Google Play. Store publication still requires the production signing key and Play Console publisher account." action="Check Android releases" href={ANDROID_RELEASES_URL} external secondary="The Play listing will be linked here once published." />
          <PlatformCard icon="🪟" title="Windows" status="PWA available now" description="Install Shadecode Student as a desktop PWA from a supported browser today. Microsoft Store distribution is a separate publishing channel." action="Open Windows-ready web app" href={WEB_URL} external secondary="Edge/Chrome → Install Shadecode Student" />
          <PlatformCard icon="🍎" title="iPhone & iPad" status="PWA available now" description="Open Shadecode Student in Safari for an app-like installed experience without waiting for App Store publication." action="Open on iPhone / iPad" href={WEB_URL} external secondary="Safari → Share → Add to Home Screen" />
          <PlatformCard icon="💻" title="Desktop PWA" status="Available now" description="On supported desktop browsers, install Shadecode Student from the browser's install control for a standalone app window." action="Open desktop app" href={WEB_URL} external secondary="Use the install icon in the address bar." />
        </section>
