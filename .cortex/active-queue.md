...
- [x] 2026-09-20: two-tier gate implemented (`CurriculumGateTier`): Learn and Cortex need the 25 syllabus dimensions; the 4 exam-history dimensions gate only exam-history claims (prompt guard when unverified). Policy: `docs/curriculum/coverage-evidence-policy.md` (specimen material is `draft` evidence only). Critical path for 9709 / 9702 is now 12 / 11 syllabus dimensions (all evidenced by the PDFs) plus the knowledge layer, using the 0478 package (75 items) as the template.
- [x] 2026-09-20: Cambridge Mathematics 9709 now FULLY resolves at the `syllabus` tier — knowledge layer (128 rows, both levels, all 15 required kinds) + the 12 remaining coverage dimensions loaded; verified against live data with the loader's exact queries (0 missing dimensions/kinds, 0 A-Level objectives leaked to AS learners). This is the first non-CS subject a real learner could be taught from, once an identity is stored.
- [x] 2026-09-26: Cambridge AS & A Level Physics 9702 knowledge layer completed: 514 verified knowledge rows, all 15 strict resolver kinds present, 25/29 syllabus-tier coverage dimensions satisfied, 300 learning outcomes mapped to canonical objective IDs, and the official PDF registered for weekly source monitoring. Four exam-history dimensions remain intentionally unverified.
