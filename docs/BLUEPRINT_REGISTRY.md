...
| Distributed Intelligence Architecture | Superseded strategic proposal | `docs/SHADECODE_DISTRIBUTED_INTELLIGENCE_ARCHITECTURE.md` | Earlier document focused primarily on distributed AI. Retained as historical context. |
| Distributed Infrastructure Architecture | Located / current strategic proposal | `docs/SHADECODE_DISTRIBUTED_INFRASTRUCTURE_ARCHITECTURE.md` | Current direction expands decentralization beyond AI to compute, storage, knowledge, synchronization, delivery, and selected services. User devices are first-class infrastructure nodes; cloud is optional infrastructure. |

## Existing implementation knowledge

- `docs/ARCHITECTURE.md` — verified current architecture plus a link to the device-native distributed infrastructure direction.
