...
## 2026-08-29 — Data-loss incident: DEVLOG.md was wholesale truncated, then restored

A commit on `main` (one of the "docs: refresh product status..." /
"docs: define canonical learning event architecture" passes on 2026-08-29)
overwrote this entire file down to 43 lines, discarding several months of
historical entries -- including everything above this note and the
correction note immediately preceding it -- while leaving the file's own
explicit instruction ("Historical entries... Do not delete them when
adding future entries") sitting directly above the deleted content.
Discovered while rebasing PR #252 onto `main`: the rebase's DEVLOG.md
conflict showed 0 bytes on `main`'s side against ~74KB on the older
branch's side at the exact same insertion point -- not a real editorial
conflict, an accidental deletion. Restored the full history here rather
than accepting the truncation, since losing an audit trail (including
today's own security audit and outage-response entries) is a real cost,
not just a formatting inconvenience. If a DEVLOG refresh is ever genuinely
warranted (e.g. splitting into dated archive files for length), that
