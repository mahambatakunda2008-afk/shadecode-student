...
drop policy if exists sync_revisions_owner_write on public.sync_revisions;
...
begin
  ...
  values(v_user,p_store,p_record_id,0,p_device_id,0,now()) on conflict (user_id,store,record_id) do nothing;
  ...
  if p_operation='delete' then
    ...
    on conflict(id) do update set subject_id=excluded.subject_id,title=excluded.title,completed=excluded.completed;
    ...
    on conflict(id) do update set subject_id=excluded.subject_id,title=excluded.title,description=excluded.description,difficulty=excluded.difficulty,progress=excluded.progress,blocks=excluded.blocks;
  end if;
