...
create index if not exists sync_revisions_user_updated_idx
  on public.sync_revisions(user_id, updated_at desc);
...
create policy sync_revisions_owner_select on public.sync_revisions
...
begin
  ...
  select version into v_current
    from public.sync_revisions
    where user_id = v_user and store = p_store and record_id = p_record_id
    for update;
  v_current := coalesce(v_current, 0);

  if p_base_version <> v_current then
    return jsonb_build_object('ok',true,'status','conflict','currentVersion',v_current,'clientVersion',p_client_version,'store',p_store,'recordId',p_record_id);
  end if;

  v_new := greatest(v_current + 1, p_client_version);

  if p_operation = 'delete' then
    execute format('delete from public.%I where id=$1 and user_id=$2', v_table) using p_record_id, v_user;
  else
    v_row := p_payload || jsonb_build_object('id',p_record_id,'user_id',v_user);
    if p_store = 'education_profile' then
      execute 'update public.user_profiles set education_stage = coalesce($3->>''educationStage'', education_stage), education_grade = coalesce(($3->>''educationGrade'')::integer, education_grade), education_year = coalesce($3->>''educationYear'', education_year), education_curriculum = coalesce($3->>''educationCurriculum'', education_curriculum), education_subjects = coalesce($3->''educationSubjects'', education_subjects), updated_at = now() where user_id=$1' using v_user, v_user, v_row;
      if not found then raise exception 'Education profile not found'; end if;
    else
      execute format('insert into public.%I select * from jsonb_populate_record(null::public.%I, $1) on conflict (id) do update set updated_at=now()', v_table, v_table) using v_row;
    end if;
  end if;

  insert into public.sync_revisions(user_id,store,record_id,version,device_id,updated_at)
  values(v_user,p_store,p_record_id,v_new,p_device_id,now())
  on conflict (user_id,store,record_id) do update set version=excluded.version,device_id=excluded.device_id,updated_at=excluded.updated_at;
